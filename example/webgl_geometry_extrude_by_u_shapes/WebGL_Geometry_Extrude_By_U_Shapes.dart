...
class WebGL_Geometry_Extrude_By_U_Shapes  {
  ...
  void init() {
    ...
    var line = new LineCurve3(new Vector3(0,0,0),new Vector3(200,0,0 ));
    var line2 = new LineCurve3(new Vector3(200,0,0 ),new Vector3(200,200,0));
    ...
    var randomPoints = [new Vector3(200,200,0)];
    ...
    var extrude_u_steps = new List();
    var curvepath_length = curvepath.length;
    num init_u = 0;
    var curve_steps = 40;
    ...
    for(var curve in curvepath.curves){
      ...
      if(curve is LineCurve3){ 
        ...
        init_u += curve.length / curvepath_length;
        extrude_u_steps.add(init_u);

      } else { 
        // Not Straight so we subdivide the curve steps in 'curve_steps' divisions
          for ( var d = 0; d <= curve_steps; d ++ ) {
            extrude_u_steps.add(init_u + ((d / curve_steps) * (curve.length / curvepath_length)) );
          }
          init_u = extrude_u_steps.last();
        }
    }
    print("[WebGL_Geometry_Extrude_By_U_Shapes] ${extrude_u_steps}");
    ...
    var tube = new TubeGeometry(extrude_extrudePath, 60, 4, 5, false, true);
