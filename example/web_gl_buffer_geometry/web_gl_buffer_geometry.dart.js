...
Chunk: {"": "Object;start,count*,index"},
...
BufferGeometry: {"": "Object;id*,attributes>,offsets<,_dynamic,boundingBox,boundingSphere<,hasTangents<,morphTargets<,morphNormals<,__data<",
  ...
  computeVertexNormals$0: function() {
    var t1, t2, t3, il, i, indices, positions, normals, pA, pB, pC, cb, ab, jl, j, vA, vB, vC, x, y, z, t4, start, index, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, n;
    t1 = this.attributes;
    if (t1.$index(t1, "position") != null && t1.$index(t1, "index") != null) {
      if (t1.$index(t1, "normal") == null) {
        t2 = t1.$index(t1, "position").get$numItems();
        t3 = new Float32Array(t2);
        t3.$dartCachedLength = t3.length;
        t1.$indexSet(t1, "normal", new S.GeometryAttribute(t2, 3, t3, null));
      } else {
        il = C.JS_CONST_ZYJ(t1.$index(t1, "normal").get$array());
        for (i = 0; i < il; ++i) {
          t2 = t1.$index(t1, "normal").get$array();
          if (i >= C.JS_CONST_ZYJ(t2))
            throw H.ioore(t2, i);
          t2[i] = 0;
        }
      }
      indices = t1.$index(t1, "index").get$array();
      positions = t1.$index(t1, "position").get$array();
      ...
      t1.$dartCachedLength = t1.length;
      ab = new T.Vector3(t1);
      jl = this.offsets.length;
      for (t1 = pA.storage, t2 = pB.storage, t3 = pC.storage, j = 0, vA = null, vB = null, vC = null, x = null, y = null, z = null; j < jl; ++j) {
        t4 = this.offsets;
        if (j >= t4.length)
          throw H.ioore(t4, j);
        t4 = t4[j];
        ...
        for (i = start; i < il; i += 3) {
          t4 = C.JS_CONST_ZYJ(indices);
          if (i < 0 || i >= t4)
            throw H.ioore(indices, i);
          vA = index + indices[i];
          t5 = i + 1;
          if (t5 >= t4)
            throw H.ioore(indices, t5);
          vB = index + indices[t5];
          t5 = i + 2;
          if (t5 >= t4)
            throw H.ioore(indices, t5);
          ...
          t11 = vC * 3;
          if (t11 >>> 0 !== t11 || t11 >= t4)
            throw H.ioore(positions, t11);
          x = positions[t11];
          t12 = t11 + 1;
          if (t12 >= t4)
            throw H.ioore(positions, t12);
          y = positions[t12];
          t13 = t11 + 2;
          if (t13 >= t4)
            throw H.ioore(positions, t13);
          z = positions[t13];
          t3[0] = x;
          t3[1] = y;
          t3[2] = z;
          ...
          ab = pA.$sub(pA, pB);
          cb = cb.cross$1(ab);
          t4 = C.JS_CONST_ZYJ(normals);
          if (t5 >= t4)
            throw H.ioore(normals, t5);
          t14 = cb.storage;
          normals[t5] = normals[t5] + t14[0];
          if (t6 >= t4)
            throw H.ioore(normals, t6);
          normals[t6] = normals[t6] + t14[1];
          if (t7 >= t4)
            throw H.ioore(normals, t7);
          normals[t7] = normals[t7] + t14[2];
          if (t8 >= t4)
            throw H.ioore(normals, t8);
          normals[t8] = normals[t8] + t14[0];
          if (t9 >= t4)
            throw H.ioore(normals, t9);
          normals[t9] = normals[t9] + t14[1];
          if (t10 >= t4)
            throw H.ioore(normals, t10);
          normals[t10] = normals[t10] + t14[2];
          if (t11 >= t4)
            throw H.ioore(normals, t11);
          normals[t11] = normals[t11] + t14[0];
          if (t12 >= t4)
            throw H.ioore(normals, t12);
          normals[t12] = normals[t12] + t14[1];
          if (t13 >= t4)
            throw H.ioore(normals, t13);
          normals[t13] = normals[t13] + t14[2];
        }
      }
      il = C.JS_CONST_ZYJ(normals);
      for (i = 0; i < il; i += 3) {
        ...
        if (t2 >= t1)
          throw H.ioore(normals, t2);
        normals[t2] = normals[t2] * n;
        if (t3 >= t1)
          throw H.ioore(normals, t3);
        normals[t3] = normals[t3] * n;
      }
      t1 = this.get$_data();
      t1.$indexSet(t1, "normalsNeedUpdate", true);
    }
  },
...
WebGLRenderer: {"": "Object;canvas,_gl,precision,_clearColor,_clearAlpha,devicePixelRatio,alpha,premultipliedAlpha,antialias,stencil,preserveDrawingBuffer,autoClear,autoClearColor,autoClearDepth,autoClearStencil,sortObjects,autoUpdateObjects,autoUpdateScene,gammaInput,gammaOutput,physicallyBasedShading,shadowMapEnabled,shadowMapAutoUpdate,shadowMapDebug,shadowMapCascade,shadowMapType,shadowMapCullFrontFaces,maxMorphTargets,maxMorphNormals,autoScaleCubemaps,renderPluginsPre,renderPluginsPost,info,_programs,_programs_counter,_currentProgram,_currentFramebuffer,_currentMaterialId,_currentGeometryGroupHash,_currentCamera,_geometryGroupCounter,_usedTextureUnits,_enabledAttributes,_oldDoubleSided,_oldFlipSided,_oldBlending,_oldBlendEquation,_oldBlendSrc,_oldBlendDst,_oldDepthTest,_oldDepthWrite,_oldPolygonOffset,_oldPolygonOffsetFactor,_oldPolygonOffsetUnits,_oldLineWidth,_viewportX,_viewportY,_viewportWidth,_viewportHeight,_currentWidth,_currentHeight,_frustum,_projScreenMatrix,_projScreenMatrixPS,_vector3,_direction,_lights,_lightsNeedUpdate,_glExtensionTextureFloat,_glExtensionStandardDerivatives,_glExtensionTextureFilterAnisotropic,_glExtensionCompressedTextureS3TC,maxAnisotropy,supportsVertexTextures,supportsBoneTextures,shadowMapPlugin,maxTextures,maxVertexTextures,maxTextureSize,maxCubemapSize",
  ...
  renderBufferDirect$6: function(camera, lights, fog, material, webglgeometry, webglobject) {
    var program, attributes, wireframeBit, t1, t2, geometryHash, updateBuffers, object, geometry, index, offsets, t3, i, startIndex, t4, position, positionSize, t5, normal, normalSize, uv, uvSize, color, colorSize, tangent, tangentSize;
    ...
    t1 = J.$mul$n(J.get$id$x(webglgeometry), 16777215);
    t2 = program.id;
    if (typeof wireframeBit !== "number")
      throw H.iae(wireframeBit);
    geometryHash = t1 + t2 * 2 + wireframeBit;
      ...
      if (index != null) {
        ...
        while (true) {
          ...
          if (updateBuffers) {
            ...
            position = t4.$index(t4, "position");
            positionSize = position.get$itemSize();
            J.get$buffer$x(position).bind$1(34962);
            this.enableAttribute$1(attributes.$index(attributes, "position"));
            t4 = this._gl;
            t5 = attributes.$index(attributes, "position");
            if (typeof 4 !== "number")
              throw H.iae(4);
            J.vertexAttribPointer$6$x(t4, t5, positionSize, 5126, false, 0, startIndex * positionSize * 4);
            ...
            if (J.$ge$n(attributes.$index(attributes, "normal"), 0) && normal != null) {
              ...
              J.vertexAttribPointer$6$x(this._gl, attributes.$index(attributes, "normal"), normalSize, 5126, false, 0, startIndex * normalSize * 4);
            }
            t4 = geometry.get$attributes(geometry);
            uv = t4.$index(t4, "uv");
            if (J.$ge$n(attributes.$index(attributes, "uv"), 0) && uv != null) {
              ...
              J.vertexAttribPointer$6$x(this._gl, attributes.$index(attributes, "uv"), uvSize, 5126, false, 0, startIndex * uvSize * 4);
            }
            t4 = geometry.get$attributes(geometry);
            color = t4.$index(t4, "color");
            if (J.$ge$n(attributes.$index(attributes, "color"), 0) && color != null) {
              ...
              J.vertexAttribPointer$6$x(this._gl, attributes.$index(attributes, "color"), colorSize, 5126, false, 0, startIndex * colorSize * 4);
            }
            t4 = geometry.get$attributes(geometry);
            tangent = t4.$index(t4, "tangent");
            if (J.$ge$n(attributes.$index(attributes, "tangent"), 0) && tangent != null) {
              ...
              J.vertexAttribPointer$6$x(this._gl, attributes.$index(attributes, "tangent"), tangentSize, 5126, false, 0, startIndex * tangentSize * 4);
            }
            t3.get$buffer(index).bind$1(34963);
          }
          t4 = this._gl;
          if (i >= offsets.length)
            throw H.ioore(offsets, i);
          t5 = offsets[i];
          J.drawElements$4$x(t4, 4, t5.count, 5123, t5.start * 2);
          t5 = t2.render;
          t5.set$calls(t5.get$calls() + 1);
          t5 = t2.render;
          t4 = t5.get$vertices();
          if (i >= offsets.length)
            throw H.ioore(offsets, i);
          t5.set$vertices(t4 + offsets[i].count);
          t4 = t2.render;
          t5 = t4.get$faces();
          if (i >= offsets.length)
            throw H.ioore(offsets, i);
          t4.set$faces(t5 + C.JSInt_methods.$tdiv(offsets[i].count, 3));
          ++i;
        }
      } else {
        position = geometry.get$aPosition();
...
WebGLMaterial: {"": "Object;_material,program@,_fragmentShader,_vertexShader,_uniforms,uniformsList@,numSupportedMorphTargets@,numSupportedMorphNormals@,shadowPass@",
  ...
  get$morphTargets: function() {
    var t1 = this._material;
    return (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isShaderMaterial) && t1.get$morphTargets();
  },
  get$morphNormals: function() {
    var t1 = this._material;
    return (!!t1.$isMeshLambertMaterial || !!t1.$isShaderMaterial) && t1.get$morphNormals();
  },
  get$metal: function() {
    return false;
  },
  ...
  get$wrapAround: function() {
    var t1 = this._material;
    return (!!t1.$isMeshLambertMaterial || false) && t1.get$wrapAround();
  },
  get$fog: function() {
  ...
  get$envMap: function() {
    var t1 = this._material;
    return !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || false ? t1.get$envMap() : null;
  },
  get$lightMap: function() {
    var t1 = this._material;
    return !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || false ? t1.get$lightMap() : null;
  },
  ...
  get$specularMap: function() {
    var t1 = this._material;
    return !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || false ? t1.get$specularMap() : null;
  },
  ...
  get$skinning: function() {
    var t1 = this._material;
    return (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isShaderMaterial) && t1.get$skinning();
  },
  ...
  get$isMeshLambertMaterial: function() {
    var t1 = this._material;
    return !!t1.$isMeshLambertMaterial;
  },
  ...
  get$_hasVertexColors: function() {
    var t1 = this._material;
    return !!t1.$isLineBasicMaterial || !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isShaderMaterial;
  },
...
["", "web_gl_buffer_geometry.dart", , M, {
main: function() {
  M.init();
  var t1 = window;
  C.Window_methods._ensureRequestAnimationFrame$0(t1);
  C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(M.animate$closure));
  M.render();
},

init: function() {
  var t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, geometry, indices, positions, i, x, y, z;
  t1 = {};
  ...
  t17 = new Float32Array(3);
  t17.$dartCachedLength = t17.length;
  t17 = new S.PerspectiveCamera(20, t2 / t3, null, null, null, null, null, null, t4, t5, t6, 1, 10000, t7, "", t8, null, [], t9, t10, t11, t12, "XYZ", null, null, null, true, null, t13, t14, t15, true, true, t16, false, 0, 1, true, false, false, true, new T.Vector3(t17), null, null);
  t17.updateProjectionMatrix$0();
  t16 = t17.position;
  t16.set$z(t16, 1800);
  $.camera = t17;
  $.scene = S.Scene$();
  t17 = $.GeometryCount;
  $.GeometryCount = t17 + 1;
  geometry = new S.BufferGeometry(t17, H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null)), [], false, null, null, null, [], [], null);
  t17 = new Int16Array(4500);
  t17.$dartCachedLength = t17.length;
  t16 = geometry.attributes;
  t16.$indexSet(t16, "index", new S.GeometryAttribute(4500, 1, t17, null));
  t17 = new Float32Array(4500);
  t17.$dartCachedLength = t17.length;
  t16.$indexSet(t16, "position", new S.GeometryAttribute(4500, 3, t17, null));
  indices = t16.$index(t16, "index").get$array();
  t1.i_0 = 0;
  J.forEach$1$ax(indices, new M.init_closure(t1, indices));
  positions = t16.$index(t16, "position").get$array();
  for (t1 = C.JS_CONST_ZYJ(positions), i = 0; i < t1; i += 9) {
    x = Math.random() * 400 - 200;
    y = Math.random() * 400 - 200;
    z = Math.random() * 400 - 200;
    t2 = Math.random();
    if (i >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, i);
    positions[i] = x + t2 * 40 - 20;
    t2 = i + 1;
    t3 = Math.random();
    if (t2 >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, t2);
    positions[t2] = y + t3 * 40 - 20;
    t3 = i + 2;
    t2 = Math.random();
    if (t3 >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, t3);
    positions[t3] = z + t2 * 40 - 20;
    t2 = i + 3;
    t3 = Math.random();
    if (t2 >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, t2);
    positions[t2] = x + t3 * 40 - 20;
    t3 = i + 4;
    t2 = Math.random();
    if (t3 >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, t3);
    positions[t3] = y + t2 * 40 - 20;
    t2 = i + 5;
    t3 = Math.random();
    if (t2 >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, t2);
    positions[t2] = z + t3 * 40 - 20;
    t3 = i + 6;
    t2 = Math.random();
    if (t3 >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, t3);
    positions[t3] = x + t2 * 40 - 20;
    t2 = i + 7;
    t3 = Math.random();
    if (t2 >= C.JS_CONST_ZYJ(positions))
      throw H.ioore(positions, t2);
  ...
  geometry.offsets = [new S.Chunk(0, 1500, 0)];
  ...
  t5 = $.MaterialCount;
  $.MaterialCount = t5 + 1;
  $.mesh = S.Mesh$(geometry, new S.MeshLambertMaterial(null, t1, t2, t3, false, t4, null, null, null, null, 0, 1, 0.98, 2, false, 1, "round", "round", false, false, false, 0, true, "", t5, 2, 1, 1, 204, 205, 100, 0, false, 0, 0, false, true, true, false, true, true, null));
  t5 = $.scene;
  t5.add$1(t5, $.mesh);
  t5 = S.WebGLRenderer$(true, true, null, 0, 0, null, "highp", true, false, true);
  t5.setSize$2(window.innerWidth, window.innerHeight);
  $.renderer = t5;
  t5 = J.get$children$x($.container);
  t5.add$1(t5, $.renderer.canvas);
  t5 = new W._EventStream(window, C.EventStreamProvider_resize._eventType, false);
  H.setRuntimeTypeInfo(t5, [null]);
  t4 = new W._EventStreamSubscription(0, t5._target, t5._eventType, W._wrapZone(M.onWindowResize$closure), t5._useCapture);
...
render: function() {
  var t1 = $.mesh.rotation.storage;
  t1[0] = t1[0] + 0.01;
  t1[1] = t1[1] + 0.02;
  $.renderer._render$2($.scene, $.camera);
},

init_closure: {"": "Closure;box_0,indices_1",
  call$1: function(_) {
    var t1, t2, t3;
    t1 = this.indices_1;
    t2 = this.box_0;
...
init.globalFunctions.onWindowResize$closure = M.onWindowResize$closure = new H.Closure$1(M.onWindowResize, "onWindowResize$closure");
init.globalFunctions.animate$closure = M.animate$closure = new H.Closure$1(M.animate, "animate$closure");
...
P.Int16List.$isIterable = true;
P.Int16List.$asIterable = [J.JSInt];
P.Int16List.$isIterable = true;
P.Int16List.$asIterable = [J.JSInt];
P.Int16List.$isObject = true;
...
//# sourceMappingURL=web_gl_buffer_geometry.dart.js.map
//@ sourceMappingURL=web_gl_buffer_geometry.dart.js.map
