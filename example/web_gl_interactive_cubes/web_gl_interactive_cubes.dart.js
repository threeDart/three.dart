...
InputElement: {"": "HtmlElement;height%,name=,size=,type=,value%,width%", $isElement: true, "%": "HTMLInputElement"},
...
extractRotation: function(te, m) {
  var t1, vector, scaleX, scaleY, scaleZ;
  t1 = new Float32Array(3);
  t1.$dartCachedLength = t1.length;
  vector = new T.Vector3(t1);
  t1 = vector.setValues$3(m.$index(m, 0), m.$index(m, 1), m.$index(m, 2));
  scaleX = 1 / t1.get$length(t1);
  t1 = vector.setValues$3(m.$index(m, 4), m.$index(m, 5), m.$index(m, 6));
  scaleY = 1 / t1.get$length(t1);
  t1 = vector.setValues$3(m.$index(m, 8), m.$index(m, 9), m.$index(m, 10));
  scaleZ = 1 / t1.get$length(t1);
  t1 = te.storage;
  t1[0] = m.$index(m, 0) * scaleX;
  t1[1] = m.$index(m, 1) * scaleX;
  t1[2] = m.$index(m, 2) * scaleX;
  t1[4] = m.$index(m, 4) * scaleY;
  t1[5] = m.$index(m, 5) * scaleY;
  t1[6] = m.$index(m, 6) * scaleY;
  t1[8] = m.$index(m, 8) * scaleZ;
  t1[9] = m.$index(m, 9) * scaleZ;
  t1[10] = m.$index(m, 10) * scaleZ;
  return te;
},
...
Frustum___v1: function() {
  if ($.Frustum____v1 == null) {
    var t1 = new Float32Array(3);
    t1.$dartCachedLength = t1.length;
    $.Frustum____v1 = new T.Vector3(t1);
  }
...
Ray: {"": "Object;origin,direction,near,far,precision",
  _distanceFromIntersection$3: function(origin, direction, position) {
    var dot, t1;
    dot = position.$sub(position, origin).dot$1(direction);
    t1 = new Float32Array(3);
    t1.$dartCachedLength = t1.length;
    t1 = new T.Vector3(t1);
    t1.setFrom$1(direction);
    t1 = position.$sub(position, origin.$add(origin, t1.scale$1(t1, dot)));
    return t1.get$length(t1);
  },
  ...
  intersectObject$2$recursive: function(object, recursive) {
    var t1, abcd, t2, originCopy, directionCopy, vector, normal, intersectPoint, intersects, distance, t3, t4, t5, scale, vertices, side, fl, intersect, dot, scalar, f, face, material, t6, pointInFace;
    t1 = {};
    abcd = P.List_List$generate(4, new S.Ray_intersectObject_closure(), true, null);
    t2 = new Float32Array(3);
    t2.$dartCachedLength = t2.length;
    originCopy = new T.Vector3(t2);
    t2 = new Float32Array(3);
    t2.$dartCachedLength = t2.length;
    directionCopy = new T.Vector3(t2);
    t2 = new Float32Array(3);
    t2.$dartCachedLength = t2.length;
    vector = new T.Vector3(t2);
    t2 = new Float32Array(3);
    t2.$dartCachedLength = t2.length;
    normal = new T.Vector3(t2);
    t2 = new Float32Array(3);
    t2.$dartCachedLength = t2.length;
    intersectPoint = new T.Vector3(t2);
    intersects = [];
    ...
    if (typeof object === "object" && object !== null && !!t2.$isMesh) {
      ...
      t2 = object.geometry;
      t3 = scale.storage;
      if (distance > t2.boundingSphere.radius * P.max(t3[0], P.max(t3[1], t3[2])))
        return intersects;
      ...
      for (intersect = null, dot = null, scalar = null, f = 0; f < fl; ++f) {
        ...
        vector.setFrom$1(face.centroid);
        vector.applyProjection$1(t1.objMatrix_0).sub$1(originCopy);
        normal.setFrom$1(face.normal);
        ...
        if (side !== 2)
          ...
          t4 = true;
        if (t4) {
          intersectPoint = originCopy.$add(originCopy, directionCopy.scale$1(directionCopy, scalar));
          t4 = new H.MappedListIterable(face.indices, new S.Ray_intersectObject_closure1(t1, vertices));
          t4.$builtinTypeInfo = [null, null];
          abcd = t4.toList$0(t4);
          t4 = abcd.length;
          if (0 >= t4)
            throw H.ioore(abcd, 0);
          t5 = abcd[0];
          if (1 >= t4)
            throw H.ioore(abcd, 1);
          t6 = abcd[1];
          if (3 >= t4)
            throw H.ioore(abcd, 3);
          if (!this._pointInFace3$4(intersectPoint, t5, t6, abcd[3])) {
            ...
            pointInFace = true;
          if (pointInFace === true) {
            t4 = originCopy.$sub(originCopy, intersectPoint);
            t4 = t4.get$length(t4);
            t5 = new Float32Array(3);
            t5.$dartCachedLength = t5.length;
            t5 = new T.Vector3(t5);
  ...
  intersectObjects$1: function(objects) {
    var intersects = [];
    H.IterableMixinWorkaround_forEach(objects, new S.Ray_intersectObjects_closure(this, intersects));
...
Ray$: function(origin, direction, near, far) {
  var t1 = new S.Ray(origin, direction, near, far, 0.0001);
...
Ray_intersectObject_closure: {"": "Closure;",
  call$1: function(_) {
    var t1 = new Float32Array(3);
    t1.$dartCachedLength = t1.length;
...
Ray_intersectObject_closure0: {"": "Closure;this_1,intersects_2",
...
Ray_intersectObject_closure1: {"": "Closure;box_0,vertices_3",
  call$1: function(idx) {
...
Ray_intersectObjects_closure: {"": "Closure;this_0,intersects_1",
  call$1: function(o) {
    return C.JSArray_methods.addAll$1(this.intersects_1, this.this_0.intersectObject$1(o));
  },
  $is_args1: true
},

Ray_intersectObjects_closure0: {"": "Closure;",
  call$2: function(a, b) {
...
WebGLRenderer: {"": "Object;canvas,_gl,precision,_clearColor,_clearAlpha,devicePixelRatio,alpha,premultipliedAlpha,antialias,stencil,preserveDrawingBuffer,autoClear,autoClearColor,autoClearDepth,autoClearStencil,sortObjects,autoUpdateObjects,autoUpdateScene,gammaInput,gammaOutput,physicallyBasedShading,shadowMapEnabled,shadowMapAutoUpdate,shadowMapDebug,shadowMapCascade,shadowMapType,shadowMapCullFrontFaces,maxMorphTargets,maxMorphNormals,autoScaleCubemaps,renderPluginsPre,renderPluginsPost,info,_programs,_programs_counter,_currentProgram,_currentFramebuffer,_currentMaterialId,_currentGeometryGroupHash,_currentCamera,_geometryGroupCounter,_usedTextureUnits,_enabledAttributes,_oldDoubleSided,_oldFlipSided,_oldBlending,_oldBlendEquation,_oldBlendSrc,_oldBlendDst,_oldDepthTest,_oldDepthWrite,_oldPolygonOffset,_oldPolygonOffsetFactor,_oldPolygonOffsetUnits,_oldLineWidth,_viewportX,_viewportY,_viewportWidth,_viewportHeight,_currentWidth,_currentHeight,_frustum,_projScreenMatrix,_projScreenMatrixPS,_vector3,_direction,_lights,_lightsNeedUpdate,_glExtensionTextureFloat,_glExtensionStandardDerivatives,_glExtensionTextureFilterAnisotropic,_glExtensionCompressedTextureS3TC,maxAnisotropy,supportsVertexTextures,supportsBoneTextures,shadowMapPlugin,maxTextures,maxVertexTextures,maxTextureSize,maxCubemapSize",
  ...
  updateObject$1: function(webglobject) {
    var object, geometry, il, t1, material, i, t2, geometryGroup, customAttributesDirty, t3;
    object = webglobject.get$object();
    geometry = webglobject.get$webglgeometry();
    if (!!object.$isMesh) {
...
Vector4: {"": "Object;storage<",
  ...
  dot$1: function(other) {
    ...
    if (3 >= t4)
      throw H.ioore(t3, 3);
    return t2 * t5 + t6 * t7 + t8 * t9 + t1 * t3[3];
  },
  add$1: function(_, arg) {
    var t1, t2, t3;
    t1 = this.storage;
...
["", "web_gl_interactive_cubes.dart", , A, {
main: function() {
  A.init();
  var t1 = window;
  C.Window_methods._ensureRequestAnimationFrame$0(t1);
  C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(A.animate$closure));
  A.render();
},

init: function() {
  var t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, light, geometry, i, object;
  ...
  t16 = new Float32Array(3);
  t16.$dartCachedLength = t16.length;
  t16 = new S.PerspectiveCamera(70, t1 / t2, null, null, null, null, null, null, t3, t4, t5, 1, 1000, t6, "", t7, null, [], t8, t9, t10, t11, "XYZ", null, null, null, true, null, t12, t13, t14, true, true, t15, false, 0, 1, true, false, false, true, new T.Vector3(t16), null, null);
  t16.updateProjectionMatrix$0();
  $.camera = t16;
  $.camera.position.setValues$3(0, 300, 500);
  $.scene = S.Scene$();
  t16 = $.scene;
  t16.add$1(t16, $.camera);
  light = S.DirectionalLight$(16777215, 2, 0);
  ...
  t16 = $.scene;
  t16.add$1(t16, light);
  geometry = S.CubeGeometry$(20, 20, 20, 1, 1, 1, null, null);
  for (i = 0; i < 500; ++i) {
    t1 = Math.random() * 16777215 >>> 0;
    ...
    t5 = $.MaterialCount;
    $.MaterialCount = t5 + 1;
    object = S.Mesh$(geometry, new S.MeshLambertMaterial(null, t2, t1, t3, false, t4, null, null, null, null, 0, 1, 0.98, 2, false, 1, "round", "round", false, false, false, 0, true, "", t5, 0, 1, 1, 204, 205, 100, 0, false, 0, 0, false, true, true, false, true, true, null));
    t5 = object.position;
    t5.storage[0] = (Math.random() * 800 >>> 0) - 400;
    t1 = object.position;
    t1.storage[1] = (Math.random() * 800 >>> 0) - 400;
    t1 = object.position;
    t1.storage[2] = (Math.random() * 800 >>> 0) - 400;
    object.rotation.storage[0] = Math.random() * 360 * 3.141592653589793 / 180;
    object.rotation.storage[1] = Math.random() * 360 * 3.141592653589793 / 180;
    object.rotation.storage[2] = Math.random() * 360 * 3.141592653589793 / 180;
  ...
  H.setRuntimeTypeInfo(t1, [null]);
  t2 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(A.onDocumentMouseMove$closure), t1._useCapture);
  H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
  t2._tryResume$0();
  t2 = new W._EventStream(window, C.EventStreamProvider_resize._eventType, false);
  H.setRuntimeTypeInfo(t2, [null]);
  t1 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(A.onWindowResize$closure), t2._useCapture);
  H.setRuntimeTypeInfo(t1, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
  t1._tryResume$0();
},
...
render: function() {
  var t1, t2, t3, vector, intersects, material;
  $.theta = $.theta + 0.2;
  t1 = $.camera.position;
  t2 = $.radius;
  t3 = $.theta;
  t1.storage[0] = t2 * Math.sin(t3 * 3.141592653589793 / 360);
  t1 = $.camera.position;
  t2 = $.radius;
  t3 = $.theta;
  t1.storage[1] = t2 * Math.sin(t3 * 3.141592653589793 / 360);
  t1 = $.camera.position;
  t2 = $.radius;
  t3 = $.theta;
  t1.storage[2] = t2 * Math.cos(t3 * 3.141592653589793 / 360);
  $.camera.lookAt$1($.scene.position);
  t1 = $.mouseX;
  t2 = $.mouseY;
  t3 = new Float32Array(3);
  t3.$dartCachedLength = t3.length;
  vector = new T.Vector3(t3);
  vector.setValues$3(t1, t2, 1);
  $.projector.unprojectVector$2(vector, $.camera);
  t2 = $.camera.position;
  t1 = vector.sub$1(t2);
  intersects = S.Ray$(t2, t1.normalize$0(t1), 0, 1 / 0).intersectObjects$1($.scene.children);
  if (intersects.length > 0) {
    t1 = $.INTERSECTED;
    if (t1 !== intersects[0].get$object()) {
      if (t1 != null) {
        t1 = H.interceptedTypeCast(t1.get$material(), "$isMeshLambertMaterial").color;
        t2 = $.INTERSECTED;
        t1.setHex$1(t2.$index(t2, "currentHex"));
      }
      if (0 >= intersects.length)
        throw H.ioore(intersects, 0);
      $.INTERSECTED = intersects[0].get$object();
      material = $.INTERSECTED.get$material();
      t1 = $.INTERSECTED;
      t1.$indexSet(t1, "currentHex", material.get$color(material).getHex$0());
      material.get$color(material).setHex$1(16711680);
    }
  } else {
    t1 = $.INTERSECTED;
    if (t1 != null) {
...
init.globalFunctions.onWindowResize$closure = A.onWindowResize$closure = new H.Closure$1(A.onWindowResize, "onWindowResize$closure");
init.globalFunctions.onDocumentMouseMove$closure = A.onDocumentMouseMove$closure = new H.Closure$1(A.onDocumentMouseMove, "onDocumentMouseMove$closure");
init.globalFunctions.animate$closure = A.animate$closure = new H.Closure$1(A.animate, "animate$closure");
...
//# sourceMappingURL=web_gl_interactive_cubes.dart.js.map
//@ sourceMappingURL=web_gl_interactive_cubes.dart.js.map
