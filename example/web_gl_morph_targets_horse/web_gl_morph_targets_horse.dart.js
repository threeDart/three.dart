...
loadCompressedTexture: function(url, mapping, onError, onLoad) {
  var texture, request, t1, t2;
  texture = new S.CompressedTexture(null, H.fillLiteralMap(["width", null, "height", null], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null)), null, null, null, null, null, null, null, null, null, 1, null, null, null, null, null, null, null, 4, [], null);
  texture.Texture$9(null, null, null, null, null, null, null, null, 1);
  texture.mapping = mapping;
  request = new XMLHttpRequest();
  t1 = new W._EventStream(request, C.EventStreamProvider_load0._eventType, false);
  H.setRuntimeTypeInfo(t1, [null]);
  t2 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new L.loadCompressedTexture_closure(onLoad, texture, request)), t1._useCapture);
  H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
  t2._tryResume$0();
  t2 = new W._EventStream(request, C.EventStreamProvider_error0._eventType, false);
  H.setRuntimeTypeInfo(t2, [null]);
  t1 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(onError), t2._useCapture);
...
parseDDS: function(buffer, loadMipmaps) {
  var dds, t1, FOURCC_DXT1, FOURCC_DXT3, FOURCC_DXT5, header, fourCC, blockBytes, dataOffset, width, height, i, dataLength, byteArray, mipmap;
  dds = H.fillLiteralMap(["mipmaps", [], "width", 0, "height", 0, "format", null, "mipmapCount", 1], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
  t1 = new L.parseDDS_fourCCToInt32();
  FOURCC_DXT1 = t1.call$1("DXT1");
  FOURCC_DXT3 = t1.call$1("DXT3");
  FOURCC_DXT5 = t1.call$1("DXT5");
  header = new Int32Array(buffer, 0, 31);
  header.$dartCachedLength = header.length;
  t1 = header.length;
  if (0 >= t1)
    throw H.ioore(header, 0);
  if (header[0] !== 542327876) {
    P.print("ImageUtils.parseDDS(): Invalid magic number in DDS header");
    return dds;
  }
  if (20 >= t1)
    throw H.ioore(header, 20);
  if ((header[20] & 4) === 0) {
    P.print("ImageUtils.parseDDS(): Unsupported format, must contain a FourCC code");
    return dds;
  }
  if (21 >= t1)
    throw H.ioore(header, 21);
  fourCC = header[21];
  if (fourCC === FOURCC_DXT1) {
    dds.$indexSet(dds, "format", 2001);
    blockBytes = 8;
  } else if (fourCC === FOURCC_DXT3) {
    dds.$indexSet(dds, "format", 2003);
    blockBytes = 16;
  } else if (fourCC === FOURCC_DXT5) {
    dds.$indexSet(dds, "format", 2004);
    blockBytes = 16;
  } else {
    P.print("ImageUtils.parseDDS(): Unsupported FourCC code: " + H.S(new L.parseDDS_int32ToFourCC().call$1(fourCC)));
    blockBytes = null;
  }
  dds.$indexSet(dds, "mipmapCount", 1);
  t1 = header.length;
  if (2 >= t1)
    throw H.ioore(header, 2);
  if ((header[2] & 131072) !== 0 && loadMipmaps) {
    if (7 >= t1)
      throw H.ioore(header, 7);
    dds.$indexSet(dds, "mipmapCount", P.max(1, header[7]));
  }
  if (4 >= header.length)
    throw H.ioore(header, 4);
  dds.$indexSet(dds, "width", header[4]);
  if (3 >= header.length)
    throw H.ioore(header, 3);
  dds.$indexSet(dds, "height", header[3]);
  if (1 >= header.length)
    throw H.ioore(header, 1);
  dataOffset = header[1] + 4;
  width = dds.$index(dds, "width");
  height = dds.$index(dds, "height");
  i = 0;
  while (true) {
    t1 = dds.$index(dds, "mipmapCount");
    if (typeof t1 !== "number")
      throw H.iae(t1);
    if (!(i < t1))
      break;
    t1 = C.JSNumber_methods.$tdiv(C.JSNumber_methods.$tdiv(P.max(4, width), 4) * P.max(4, height), 4);
    if (typeof blockBytes !== "number")
      throw H.iae(blockBytes);
    dataLength = t1 * blockBytes;
    byteArray = new Uint8Array(buffer, dataOffset, dataLength);
    byteArray.$dartCachedLength = byteArray.length;
    mipmap = H.fillLiteralMap(["data", byteArray, "width", width, "height", height], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
    J.add$1$ax(dds.$index(dds, "mipmaps"), mipmap);
    dataOffset += dataLength;
    width = P.max(J.$mul$n(width, 0.5), 1);
    height = P.max(J.$mul$n(height, 0.5), 1);
    ++i;
  }
  return dds;
},

loadCompressedTexture_closure: {"": "Closure;onLoad_0,texture_1,request_2",
  call$1: function(e) {
    var dds, t1;
    dds = L.parseDDS(W._convertNativeToDart_XHR_Response(this.request_2.response), true);
    t1 = this.texture_1;
    t1.format = dds.$index(dds, "format");
    t1.mipmaps = dds.$index(dds, "mipmaps");
    J.set$width$x(t1.image, dds.$index(dds, "width"));
    J.set$height$x(t1.image, dds.$index(dds, "height"));
...
parseDDS_fourCCToInt32: {"": "Closure;",
  call$1: function(value) {
    var t1, t2;
    t1 = value.charCodeAt$1(0);
    t2 = value.charCodeAt$1(1);
    t2 = t1.$add(t1, t2.$shl(t2, 8));
    t1 = value.charCodeAt$1(2);
    t1 = t2.$add(t2, t1.$shl(t1, 16));
    t2 = value.charCodeAt$1(3);
    return t1.$add(t1, t2.$shl(t2, 24));
  },
  $is_args1: true
},

parseDDS_int32ToFourCC: {"": "Closure;",
  call$1: function(value) {
    if (typeof value !== "number")
      throw value.$and();
    return P.String_String$fromCharCodes([value & 255, C.JSNumber_methods.$shr(value, 8) & 255, C.JSNumber_methods.$shr(value, 16) & 255, C.JSNumber_methods.$shr(value, 24) & 255]);
  },
  $is_args1: true
}}],
...
CssRule: {"": "Interceptor;type=", "%": "CSSCharsetRule|CSSHostRule|CSSImportRule|CSSKeyframesRule|CSSMediaRule|CSSSupportsRule|CSSUnknownRule|MozCSSKeyframesRule|WebKitCSSKeyframesRule|WebKitCSSRegionRule;CSSRule"},
...
FontFace: {"": "Interceptor;style=", "%": "FontFace"},
...
ImageBitmap: {"": "Interceptor;height=,width=", "%": "ImageBitmap"},
...
InputElement: {"": "HtmlElement;height%,name=,size=,type=,value%,width%", $isElement: true, $isNode: true, "%": "HTMLInputElement"},
...
Node: {"": "EventTarget;lastChild=,nodeType=,parent:parentElement=,previousNode:previousSibling=",
...
ScriptElement0: {"": "HtmlElement;crossOrigin},src},type=", "%": "HTMLScriptElement"},
...
TreeWalker: {"": "Interceptor;",
  ...
  get$lastChild: function(_receiver) {
    return new W.BoundClosure$i0(this, W.TreeWalker.prototype.lastChild$0, _receiver, "lastChild$0");
  },
  "%": "TreeWalker"
},
...
Console: {"": "Object;",
  ...
  get$error: function(_receiver) {
    return new J.BoundClosure$i10(this, W.Console.prototype.error$1, _receiver, "error$1");
  },
  static: {
...
Matrix: {"": "Interceptor;a=,b=,c=,d=", "%": "SVGMatrix"},
...
Loader_addStatusElement: function() {
  var e, t1, t2;
  e = W._ElementFactoryProvider_createElement_tag("div", null);
  t1 = J.getInterceptor$x(e);
  t2 = t1.get$style(e);
  J.getInterceptor$x(t2).set$position(t2, "absolute");
  C.CssStyleDeclaration_methods.set$right(t2, "0px");
  C.CssStyleDeclaration_methods.set$top(t2, "0px");
  C.CssStyleDeclaration_methods.set$fontSize(t2, "0.8em");
  C.CssStyleDeclaration_methods.set$textAlign(t2, "left");
  C.CssStyleDeclaration_methods.set$background(t2, "rgba(0,0,0,0.25)");
  C.CssStyleDeclaration_methods.set$color(t2, "#fff");
  C.CssStyleDeclaration_methods.set$width(t2, "120px");
  C.CssStyleDeclaration_methods.set$padding(t2, "0.5em 0.5em 0.5em 0.5em");
  C.CssStyleDeclaration_methods.set$zIndex(t2, "1000");
...
Loader__extractUrlBase: function(url) {
  ...
  if (0 >= parts.length)
    throw H.ioore(parts, 0);
  parts.pop();
  return (parts.length < 1 ? "." : C.JSArray_methods.join$1(parts, "/")) + "/";
},
    ...
    for (t1 = this._frustum, i = 0, j = null, jl = null, shadowMap = null, shadowMatrix = null, shadowCamera = null, buffer = null, material = null, webglObject = null, object = null, renderList = null; i < il; ++i) {
      ...
      for (; j < jl; ++j) {
        ...
        if (webglObject.get$render() === true) {
          ...
          useMorphing = J.get$length$asx(object.get$geometry().get$morphTargets()) > 0 && objectMaterial.get$morphTargets();
...
Color: {"": "Object;_r,_g,_b",
  ...
  setHex$1: function(hex) {
    var h = C.JSInt_methods.toInt$0(J.floor$0$n(hex));
...
Face: {"": "Object;normal@,vertexNormals<,vertexColors<,color>,centroid<",
  get$size: function(_) {
...
Geometry: {"": "Object;id*,name,vertices@,colors<,normals,materials,faces<,faceUvs<,faceVertexUvs,morphTargets<,morphColors,morphNormals<,skinWeights<,skinIndices<,lineDistances<,__tmpVertices,boundingBox,boundingSphere<,hasTangents<,_dynamic,bones,animation,__data<",
  ...
  computeTangents$0: function() {
    var t1, t2, t3, t4, t5, t6, tan1, tan2, handleTriangle, fl, f, triangles;
    t1 = {};
    t1.face_0 = null;
    t1.i_1 = null;
    t1.il_2 = null;
    t1.vertexIndex_3 = null;
    t1.test_4 = null;
    t1.w_5 = null;
    t1.vA_6 = null;
    t1.vB_7 = null;
    ...
    t5 = new Float32Array(3);
    t5.$dartCachedLength = t5.length;
    t1.tmp2_23 = new T.Vector3(t5);
    t5 = new Float32Array(3);
    t5.$dartCachedLength = t5.length;
    t1.t_24 = null;
    t6 = J.map$1$ax(this.vertices, new S.Geometry_computeTangents_closure());
    tan1 = t6.toList$0(t6);
    t6 = J.map$1$ax(this.vertices, new S.Geometry_computeTangents_closure0());
    tan2 = t6.toList$0(t6);
    handleTriangle = new S.Geometry_computeTangents_closure1(t1, null, new T.Vector3(t2), new T.Vector3(t3), tan1, tan2);
    fl = this.faces.length;
    for (f = 0; t2 = this.faces, f < fl; ++f) {
      if (f >= t2.length)
        throw H.ioore(t2, f);
      t1.face_0 = t2[f];
      ...
      if (f >= t2.length)
        throw H.ioore(t2, f);
      triangles = [];
      t2 = t1.face_0.indices.length;
      if (t2 === 3)
        triangles.push([0, 1, 2]);
      else if (t2 === 4) {
        triangles.push([0, 1, 3]);
        triangles.push([1, 2, 3]);
      }
      H.IterableMixinWorkaround_forEach(triangles, new S.Geometry_computeTangents_closure2(t1, this, handleTriangle));
    }
    H.IterableMixinWorkaround_forEach(t2, new S.Geometry_computeTangents_closure3(t1, new T.Vector3(t4), new T.Vector3(t5), tan1, tan2));
...
Geometry_computeTangents_closure: {"": "Closure;",
  call$1: function(_) {
    var t1 = new Float32Array(3);
    t1.$dartCachedLength = t1.length;
...
Geometry_computeTangents_closure0: {"": "Closure;",
  call$1: function(_) {
    var t1 = new Float32Array(3);
    t1.$dartCachedLength = t1.length;
    return new T.Vector3(t1);
...
Geometry_computeTangents_closure1: {"": "Closure;box_0,uv_1,sdir_2,tdir_3,tan1_4,tan2_5",
  call$7: function(context, a, b, c, ua, ub, uc) {
    var t1, t2, t3, t4, t5, t6, t7, t8;
    t1 = this.box_0;
    t1.vA_6 = J.$index$asx(context.get$vertices(), a);
    t1.vB_7 = J.$index$asx(context.get$vertices(), b);
    t1.vC_8 = J.$index$asx(context.get$vertices(), c);
    t2 = this.uv_1;
    t1.uvA_9 = C.JSNull_methods.$index(t2, ua);
    t1.uvB_10 = C.JSNull_methods.$index(t2, ub);
    t1.uvC_11 = C.JSNull_methods.$index(t2, uc);
    t2 = J.get$x$x(t1.vB_7);
    t3 = J.get$x$x(t1.vA_6);
    if (typeof t2 !== "number")
      throw t2.$sub();
    if (typeof t3 !== "number")
      throw H.iae(t3);
    t1.x1_12 = t2 - t3;
    t3 = J.get$x$x(t1.vC_8);
    t2 = J.get$x$x(t1.vA_6);
    if (typeof t3 !== "number")
      throw t3.$sub();
    if (typeof t2 !== "number")
      throw H.iae(t2);
    t1.x2_13 = t3 - t2;
    t2 = J.get$y$x(t1.vB_7);
    t3 = J.get$y$x(t1.vA_6);
    if (typeof t2 !== "number")
      throw t2.$sub();
    if (typeof t3 !== "number")
      throw H.iae(t3);
    t1.y1_14 = t2 - t3;
    t3 = J.get$y$x(t1.vC_8);
    t2 = J.get$y$x(t1.vA_6);
    if (typeof t3 !== "number")
      throw t3.$sub();
    if (typeof t2 !== "number")
      throw H.iae(t2);
    t1.y2_15 = t3 - t2;
    t2 = J.get$z$x(t1.vB_7);
    t3 = J.get$z$x(t1.vA_6);
    if (typeof t2 !== "number")
      throw t2.$sub();
    if (typeof t3 !== "number")
      throw H.iae(t3);
    t1.z1_16 = t2 - t3;
    t3 = J.get$z$x(t1.vC_8);
    t2 = J.get$z$x(t1.vA_6);
    if (typeof t3 !== "number")
      throw t3.$sub();
    if (typeof t2 !== "number")
      throw H.iae(t2);
    t1.z2_17 = t3 - t2;
    t2 = t1.uvB_10.get$u();
    t1.s1_18 = t2.$sub(t2, t1.uvA_9.get$u());
    t2 = t1.uvC_11.get$u();
    t1.s2_19 = t2.$sub(t2, t1.uvA_9.get$u());
    t2 = t1.uvB_10.get$v();
    t1.t1_20 = t2.$sub(t2, t1.uvA_9.get$v());
    t2 = t1.uvC_11.get$v();
    t1.t2_21 = t2.$sub(t2, t1.uvA_9.get$v());
    t2 = C.JSNull_methods.$mul(t1.s1_18, t1.t2_21);
    t1.r_22 = C.JSInt_methods.$div(1, t2.$sub(t2, C.JSNull_methods.$mul(t1.s2_19, t1.t1_20)));
    t2 = this.sdir_2;
    t3 = t1.t2_21;
    t4 = C.JSNull_methods.$mul(t3, t1.x1_12);
    t5 = t1.t1_20;
    t4 = t4.$sub(t4, C.JSNull_methods.$mul(t5, t1.x2_13));
    t6 = t1.r_22;
    t4 = t4.$mul(t4, t6);
    t7 = C.JSNull_methods.$mul(t3, t1.y1_14);
    t7 = t7.$sub(t7, C.JSNull_methods.$mul(t5, t1.y2_15));
    t7 = t7.$mul(t7, t6);
    t3 = C.JSNull_methods.$mul(t3, t1.z1_16);
    t5 = t3.$sub(t3, C.JSNull_methods.$mul(t5, t1.z2_17));
    t2.setValues$3(t4, t7, t5.$mul(t5, t6));
    t6 = this.tdir_3;
    t5 = t1.s1_18;
    t7 = C.JSNull_methods.$mul(t5, t1.x2_13);
    t4 = t1.s2_19;
    t7 = t7.$sub(t7, C.JSNull_methods.$mul(t4, t1.x1_12));
    t3 = t1.r_22;
    t7 = t7.$mul(t7, t3);
    t8 = C.JSNull_methods.$mul(t5, t1.y2_15);
    t8 = t8.$sub(t8, C.JSNull_methods.$mul(t4, t1.y1_14));
    t8 = t8.$mul(t8, t3);
    t5 = C.JSNull_methods.$mul(t5, t1.z2_17);
    t1 = t5.$sub(t5, C.JSNull_methods.$mul(t4, t1.z1_16));
    t6.setValues$3(t7, t8, t1.$mul(t1, t3));
    t3 = this.tan1_4;
    if (a >>> 0 !== a || a >= t3.length)
      throw H.ioore(t3, a);
    J.add$1$ax(t3[a], t2);
    if (b >>> 0 !== b || b >= t3.length)
      throw H.ioore(t3, b);
    J.add$1$ax(t3[b], t2);
    if (c >>> 0 !== c || c >= t3.length)
      throw H.ioore(t3, c);
    J.add$1$ax(t3[c], t2);
    t2 = this.tan2_5;
    if (a >= t2.length)
...
Geometry_computeTangents_closure2: {"": "Closure;box_0,this_6,handleTriangle_7",
  call$1: function(t) {
    var t1, t2, t3, t4, t5;
    t1 = this.box_0;
    t2 = t1.face_0.indices;
    ...
    if (t5 >>> 0 !== t5 || t5 >= t2.length)
      throw H.ioore(t2, t5);
    t5 = t2[t5];
    t1 = t1.face_0.indices;
    t2 = t3.$index(t, 2);
    if (t2 >>> 0 !== t2 || t2 >= t1.length)
      throw H.ioore(t1, t2);
    this.handleTriangle_7.call$7(this.this_6, t4, t5, t1[t2], t3.$index(t, 0), t3.$index(t, 1), t3.$index(t, 2));
  },
  $is_args1: true
},

Geometry_computeTangents_closure3: {"": "Closure;box_0,tmp_8,n_9,tan1_10,tan2_11",
  call$1: function(face) {
    var t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16;
    t1 = this.box_0;
    t1.il_2 = face.get$vertexNormals().length;
    t1.i_1 = 0;
    t2 = this.tmp_8;
    t3 = t2.storage;
    t4 = this.n_9;
    ...
    while (true) {
      ...
      if (t8 >>> 0 !== t8 || t8 >= t5.length)
        throw H.ioore(t5, t8);
      t1.vertexIndex_3 = t5[t8];
      t8 = t1.vertexIndex_3;
      if (t8 >>> 0 !== t8 || t8 >= t6.length)
        throw H.ioore(t6, t8);
      t1.t_24 = t6[t8];
      t2.setFrom$1(t1.t_24);
      t8 = t2.sub$1(t4.scale$1(t4, t4.dot$1(t1.t_24)));
      t8.normalize$0(t8);
      t8 = face.vertexNormals;
      t9 = t1.i_1;
      if (t9 >>> 0 !== t9 || t9 >= t8.length)
        throw H.ioore(t8, t9);
      t1.tmp2_23 = t8[t9].cross$1(t1.t_24);
      t9 = t1.tmp2_23;
      t8 = t1.vertexIndex_3;
      if (t8 >>> 0 !== t8 || t8 >= t7.length)
        throw H.ioore(t7, t8);
      t1.test_4 = t9.dot$1(t7[t8]);
      t8 = t1.test_4;
      if (typeof t8 !== "number")
        throw t8.$lt();
      t1.w_5 = t8 < 0 ? -1 : 1;
      t9 = face.vertexTangents;
      t10 = t1.i_1;
      t11 = t3[0];
      t12 = t3[1];
      t13 = t3[2];
...
MorphColors: {"": "Object;name,colors<"},

MorphTarget: {"": "Object;name,vertices@"},
...
JSONLoader: {"": "Loader;statusDomElement,onLoadStart,onLoadProgress,onLoadComplete,crossOrigin",
  load$3$texturePath: function(_, url, callback, texturePath) {
    texturePath = S.Loader__extractUrlBase(url);
    this.onLoadStart$0(this);
    this._loadAjaxJSON$3(url, callback, texturePath);
  },
  load$2: function($receiver, url, callback) {
    return this.load$3$texturePath($receiver, url, callback, null);
  },
  _loadAjaxJSON$4$callbackProgress: function(url, callback, texturePath, callbackProgress) {
    var t1, xhr, t2;
    t1 = {};
    xhr = new XMLHttpRequest();
    t1.length_0 = 0;
    t2 = new W._EventStream(xhr, C.EventStreamProvider_readystatechange._eventType, false);
    H.setRuntimeTypeInfo(t2, [null]);
    t1 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(new S.JSONLoader__loadAjaxJSON_closure(t1, this, url, callback, texturePath, callbackProgress, xhr)), t2._useCapture);
    H.setRuntimeTypeInfo(t1, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
    t1._tryResume$0();
    C.HttpRequest_methods.open$2(xhr, "GET", url);
    xhr.send(null);
  },
  _loadAjaxJSON$3: function(url, callback, texturePath) {
    return this._loadAjaxJSON$4$callbackProgress(url, callback, texturePath, null);
  },
  _createModel$3: function(json, callback, texturePath) {
    var geometry, t1, t2, scale;
    geometry = S.Geometry$();
    t1 = J.getInterceptor$x(json);
    if (t1.containsKey$1(json, "scale") === true) {
      t2 = t1.$index(json, "scale");
      if (typeof t2 !== "number")
        throw H.iae(t2);
      scale = 1 / t2;
    } else
      scale = 1;
    this._initMaterials$3(geometry, t1.$index(json, "materials"), texturePath);
    this._parseModel$3(json, geometry, scale);
    this._parseSkin$2(json, geometry);
    this._parseMorphing$3(json, geometry, scale);
    geometry.computeCentroids$0();
    geometry.computeFaceNormals$0();
    if (this._hasNormals$1(geometry))
      geometry.computeTangents$0();
    callback.call$1(geometry);
  },
  _parseModel$3: function(json, geometry, scale) {
    var t1, faces, vertices, normals, colors, i, nUvLayers, t2, t3, t4, zLength, offset, vertex, offset0, j, colorIndex, normalIndex, uvIndex, materialIndex, color, normal, uvLayer, uvs, u, v, type, hasFaceUv, t5, face, t6, nVertices, fi, normalIndex0;
    t1 = J.getInterceptor$asx(json);
    faces = t1.$index(json, "faces");
    vertices = t1.$index(json, "vertices");
    normals = t1.$index(json, "normals");
    colors = t1.$index(json, "colors");
    i = 0;
    nUvLayers = 0;
    while (true) {
      t2 = J.get$length$asx(t1.$index(json, "uvs"));
      if (typeof t2 !== "number")
        throw H.iae(t2);
      if (!(i < t2))
        break;
      if (J.get$isEmpty$asx(J.$index$asx(t1.$index(json, "uvs"), i)) !== true)
        ++nUvLayers;
      ++i;
    }
    geometry.faceUvs = P.List_List(nUvLayers, null);
    geometry.faceVertexUvs = P.List_List(nUvLayers, null);
    for (t2 = J.getInterceptor$asx(faces), i = 0; i < nUvLayers; ++i) {
      t3 = geometry.faceUvs;
      t4 = P.List_List(t2.get$length(faces), null);
      if (i >= t3.length)
        throw H.ioore(t3, i);
      t3[i] = t4;
      t4 = geometry.faceVertexUvs;
      t3 = P.List_List(t2.get$length(faces), null);
      if (i >= t4.length)
        throw H.ioore(t4, i);
      t4[i] = t3;
    }
    t3 = J.getInterceptor$asx(vertices);
    zLength = t3.get$length(vertices);
    if (typeof zLength !== "number")
      throw H.iae(zLength);
    offset = 0;
    for (; offset < zLength; offset = offset0) {
      t4 = new Float32Array(3);
      t4.$dartCachedLength = t4.length;
      vertex = new T.Vector3(t4);
      offset0 = offset + 1;
      t4 = vertex.storage;
      t4[0] = J.$mul$n(t3.$index(vertices, offset), scale);
      offset = offset0 + 1;
      t4[1] = J.$mul$n(t3.$index(vertices, offset0), scale);
      offset0 = offset + 1;
      t4[2] = J.$mul$n(t3.$index(vertices, offset), scale);
      J.add$1$ax(geometry.vertices, vertex);
    }
    zLength = t2.get$length(faces);
    if (typeof zLength !== "number")
      throw H.iae(zLength);
    t3 = J.getInterceptor$asx(normals);
    t4 = J.getInterceptor$asx(colors);
    j = null;
    offset = 0;
    colorIndex = null;
    normalIndex = null;
    ...
    for (; offset < zLength;) {
      ...
      if ((type & 1) > 0) {
        t5 = [0, 0, 0, 0];
        face = new S.Face4(t5, null, null, null, null, null, null, null);
        face.Face$4(t5, null, null, null);
        offset0 = t2.$index(faces, offset0);
        t5 = face.indices;
        if (0 >= t5.length)
          throw H.ioore(t5, 0);
        t5[0] = offset0;
        offset0 = offset + 1;
        t6 = t2.$index(faces, offset);
        if (1 >= t5.length)
          throw H.ioore(t5, 1);
        t5[1] = t6;
        offset = offset0 + 1;
        ...
        t5 = [0, 0, 0];
        face = new S.Face3(t5, null, null, null, null, null, null, null);
        face.Face$4(t5, null, null, null);
        offset0 = t2.$index(faces, offset0);
        t5 = face.indices;
        if (0 >= t5.length)
      ...
      if ((type & 2) > 0) {
        offset0 = offset + 1;
        materialIndex = t2.$index(faces, offset);
        face.materialIndex = materialIndex;
        offset = offset0;
      }
      fi = geometry.faces.length;
      if (hasFaceUv)
        for (i = 0; i < nUvLayers; ++i, offset = offset0) {
          uvLayer = J.$index$asx(t1.$index(json, "uvs"), i);
          offset0 = offset + 1;
          uvIndex = t2.$index(faces, offset);
          t5 = J.getInterceptor$n(uvIndex);
          t6 = J.getInterceptor$asx(uvLayer);
          u = t6.$index(uvLayer, t5.$mul(uvIndex, 2));
          v = t6.$index(uvLayer, J.$add$ns(t5.$mul(uvIndex, 2), 1));
          t5 = geometry.faceUvs;
          if (i >= t5.length)
            throw H.ioore(t5, i);
          t5 = t5[i];
      ...
      if ((type & 8) > 0)
        for (i = 0; i < nUvLayers; ++i) {
          uvLayer = J.$index$asx(t1.$index(json, "uvs"), i);
          uvs = P.List_List(nVertices, null);
          for (t5 = J.getInterceptor$asx(uvLayer), j = 0; j < nVertices; ++j, offset = offset0) {
            offset0 = offset + 1;
            uvIndex = t2.$index(faces, offset);
            t6 = J.getInterceptor$n(uvIndex);
            u = t5.$index(uvLayer, t6.$mul(uvIndex, 2));
            v = t5.$index(uvLayer, J.$add$ns(t6.$mul(uvIndex, 2), 1));
            uvs[j] = new S.UV(u, v);
          }
          t5 = geometry.faceVertexUvs;
          if (i >= t5.length)
            throw H.ioore(t5, i);
          t5 = t5[i];
          if (fi >= t5.length)
      ...
      if ((type & 16) > 0) {
        offset0 = offset + 1;
        normalIndex = J.$mul$n(t2.$index(faces, offset), 3);
        t5 = new Float32Array(3);
        t5.$dartCachedLength = t5.length;
        normal = new T.Vector3(t5);
        normalIndex0 = J.$add$ns(normalIndex, 1);
        t5 = normal.storage;
        t5[0] = t3.$index(normals, normalIndex);
        normalIndex = J.$add$ns(normalIndex0, 1);
        t5[1] = t3.$index(normals, normalIndex0);
        t5[2] = t3.$index(normals, normalIndex);
        face.normal = normal;
        offset = offset0;
      }
      if ((type & 32) > 0)
        for (i = 0; i < nVertices; ++i, offset = offset0) {
          offset0 = offset + 1;
          normalIndex = J.$mul$n(t2.$index(faces, offset), 3);
          t5 = new Float32Array(3);
          t5.$dartCachedLength = t5.length;
          ...
          t5[1] = t3.$index(normals, normalIndex0);
          t5[2] = t3.$index(normals, normalIndex);
          C.JSArray_methods.add$1(face.vertexNormals, normal);
        }
      if ((type & 64) > 0) {
        offset0 = offset + 1;
        colorIndex = t2.$index(faces, offset);
        t5 = t4.$index(colors, colorIndex);
        color = new S.Color(1, 1, 1);
        if (typeof t5 === "number")
          color.setHex$1(t5);
        face.color = color;
        offset = offset0;
      ...
      if ((type & 128) > 0)
        for (i = 0; i < nVertices; ++i, offset = offset0) {
          ...
          if (typeof t5 === "number")
            color.setHex$1(t5);
          C.JSArray_methods.add$1(face.vertexColors, color);
        }
      geometry.faces.push(face);
    }
  },
  _parseSkin$2: function(json, geometry) {
    var t1, l, t2, i, x, y, t3, t4, a, b;
    t1 = J.getInterceptor$x(json);
    if (t1.containsKey$1(json, "skinWeights") === true) {
      l = J.get$length$asx(t1.$index(json, "skinWeights"));
      if (typeof l !== "number")
        throw H.iae(l);
      t2 = geometry.skinWeights;
      i = 0;
      for (; i < l; i += 2) {
        x = J.$index$asx(t1.$index(json, "skinWeights"), i);
        y = J.$index$asx(t1.$index(json, "skinWeights"), i + 1);
        t3 = new Float32Array(4);
        t3.$dartCachedLength = t3.length;
        t3 = new T.Vector4(t3);
        t4 = t3.storage;
    ...
    if (t1.containsKey$1(json, "skinIndices") === true) {
      l = J.get$length$asx(t1.$index(json, "skinIndices"));
      if (typeof l !== "number")
        throw H.iae(l);
      t2 = geometry.skinIndices;
      i = 0;
      for (; i < l; i += 2) {
        a = J.$index$asx(t1.$index(json, "skinIndices"), i);
        b = J.$index$asx(t1.$index(json, "skinIndices"), i + 1);
        t3 = new Float32Array(4);
        t3.$dartCachedLength = t3.length;
        t3 = new T.Vector4(t3);
        t4 = t3.storage;
    ...
    geometry.bones = t1.$index(json, "bones");
    geometry.animation = t1.$index(json, "animation");
  },
  _parseMorphing$3: function(json, geometry, scale) {
    var t1, i, t2, t3, dstVertices, srcVertices, vl, v, t4, vertex, color, dstColors, srcColors, cl, c, t5;
    t1 = J.getInterceptor$x(json);
    if (t1.containsKey$1(json, "morphTargets") === true) {
      geometry.morphTargets = P.List_List(J.get$length$asx(t1.$index(json, "morphTargets")), null);
      for (i = 0; t2 = geometry.morphTargets, i < t2.length; ++i) {
        t3 = J.$index$asx(J.$index$asx(t1.$index(json, "morphTargets"), i), "name");
        if (i >= t2.length)
          throw H.ioore(t2, i);
        t2[i] = new S.MorphTarget(t3, []);
        t3 = geometry.morphTargets;
        if (i >= t3.length)
          throw H.ioore(t3, i);
        dstVertices = t3[i].vertices;
        srcVertices = J.$index$asx(J.$index$asx(t1.$index(json, "morphTargets"), i), "vertices");
        t3 = J.getInterceptor$asx(srcVertices);
        vl = t3.get$length(srcVertices);
        if (typeof vl !== "number")
          throw H.iae(vl);
        t2 = J.getInterceptor$ax(dstVertices);
        v = 0;
        for (; v < vl; v += 3) {
          t4 = new Float32Array(3);
          t4.$dartCachedLength = t4.length;
          vertex = new T.Vector3(t4);
          t4 = vertex.storage;
          t4[0] = J.$mul$n(t3.$index(srcVertices, v), scale);
          t4[1] = J.$mul$n(t3.$index(srcVertices, v + 1), scale);
          t4[2] = J.$mul$n(t3.$index(srcVertices, v + 2), scale);
          t2.add$1(dstVertices, vertex);
        }
      }
    }
    if (t1.containsKey$1(json, "morphColors") === true) {
      geometry.morphColors = P.List_List(J.get$length$asx(t1.$index(json, "morphColors")), null);
      for (i = 0, color = null; i < geometry.morphColors.length; ++i) {
        dstColors = [];
        srcColors = J.$index$asx(J.$index$asx(t1.$index(json, "morphColors"), i), "colors");
        t2 = J.getInterceptor$asx(srcColors);
        cl = t2.get$length(srcColors);
        if (typeof cl !== "number")
          throw H.iae(cl);
        c = 0;
        for (; c < cl; c += 3) {
          color = new S.Color(1, 1, 1);
          color.setHex$1(16755200);
          t3 = t2.$index(srcColors, c);
          t4 = t2.$index(srcColors, c + 1);
          t5 = t2.$index(srcColors, c + 2);
          color._r = J.toDouble$0$n(t3);
          color._g = J.toDouble$0$n(t4);
          color._b = J.toDouble$0$n(t5);
          dstColors.push(color);
        }
        t2 = geometry.morphColors;
        t3 = J.$index$asx(J.$index$asx(t1.$index(json, "morphColors"), i), "name");
        if (i >= t2.length)
          throw H.ioore(t2, i);
        t2[i] = new S.MorphColors(t3, dstColors);
      }
    }
  }
},

JSONLoader__loadAjaxJSON_closure: {"": "Closure;box_0,this_1,url_2,callback_3,texturePath_4,callbackProgress_5,xhr_6",
  call$1: function(e) {
    var t1, t2;
    t1 = this.xhr_6;
    t2 = t1.readyState;
    if (t2 == null ? 4 == null : t2 === 4) {
      ...
      if (t2 === 200 || t2 === 0) {
        t1 = t1.responseText;
        if (t1.length !== 0)
          this.this_1._createModel$3(C.C_JsonCodec.decode$1(t1), this.callback_3, this.texturePath_4);
        else
          P.print("THREE.JSONLoader: [" + this.url_2 + "] seems to be unreachable or file there is empty");
        this.this_1.onLoadComplete$0();
      } else
        P.print("THREE.JSONLoader: Couldn't load [" + this.url_2 + "] [" + H.S(t2) + "]");
    } else if (t2 == null ? 3 == null : t2 === 3)
      ;
    else if (t2 == null ? 2 == null : t2 === 2)
      this.box_0.length_0 = t1.getResponseHeader("Content-Length");
  },
  $is_args1: true
},

Loader: {"": "Object;",
  onLoadStart$0: function($receiver) {
    return this.onLoadStart.call$0();
  },
  onLoadComplete$0: function() {
    return this.onLoadComplete.call$0();
  },
  _initMaterials$3: function(geometry, materials, texturePath) {
    var t1, i, t2;
    geometry.materials = [];
    t1 = J.getInterceptor$asx(materials);
    i = 0;
    while (true) {
      t2 = t1.get$length(materials);
      if (typeof t2 !== "number")
        throw H.iae(t2);
      if (!(i < t2))
        break;
      geometry.materials.push(this._createMaterial$2(t1.$index(materials, i), texturePath));
      ++i;
    }
  },
  _hasNormals$1: function(geometry) {
    var t1, il, i, m;
  ...
  _is_pow2$1: function(n) {
    var l;
    if (typeof n !== "number")
      H.throwExpression(new P.ArgumentError(n));
    l = Math.log(n) / 0.6931471805599453;
    return C.JSNumber_methods.toInt$0(Math.floor(l)) === l;
  },
  _nearest_pow2$1: function(n) {
    var t1;
    if (typeof n !== "number")
      H.throwExpression(new P.ArgumentError(n));
    t1 = C.JSNumber_methods.toInt$0(C.JSDouble_methods.roundToDouble$0(Math.log(n) / 0.6931471805599453));
    return Math.pow(2, t1);
  },
  _load_image$2: function(where, url) {
    var image, t1, t2, t3;
    image = W.ImageElement_ImageElement(null, null, null);
    t1 = J.getInterceptor$x(image);
    t2 = t1.get$onLoad(image);
    t3 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(new S.Loader__load_image_closure(this, where, image)), t2._useCapture);
    H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
    t3._tryResume$0();
    t1.set$crossOrigin(image, this.crossOrigin);
    t1.set$src(image, url);
  },
  _create_texture$6: function(texturePath, sourceFile, repeat, offset, wrap, anisotropy) {
    var isCompressed, fullPath, result, t1, t2, t3, t4, wrapMap;
    isCompressed = C.JSString_methods.endsWith$1(J.toLowerCase$0$s(sourceFile), ".dds");
    fullPath = J.$add$ns(texturePath, "/") + sourceFile;
    result = isCompressed ? L.loadCompressedTexture(fullPath, null, null, null) : S.Texture$(W._ElementFactoryProvider_createElement_tag("canvas", null), null, 1, 1, 6, 8, 18, 10, 1);
    if (repeat === true) {
      t1 = result.get$repeat();
      t2 = J.getInterceptor$asx(repeat);
      t3 = t2.$index(repeat, 0);
      t4 = t2.$index(repeat, 1);
      t1 = t1.storage;
      t1[0] = t3;
      t1[1] = t4;
      if (!J.$eq(t2.$index(repeat, 0), 1))
        result.wrapS = 0;
      if (!J.$eq(t2.$index(repeat, 1), 1))
        result.wrapT = 0;
    }
    if (offset === true) {
      t1 = result.get$offset(result);
      t2 = J.getInterceptor$asx(offset);
      t3 = t2.$index(offset, 0);
      t2 = t2.$index(offset, 1);
      t1 = t1.storage;
      t1[0] = t3;
      t1[1] = t2;
    }
    if (wrap === true) {
      wrapMap = H.fillLiteralMap(["repeat", 0, "mirror", 2], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
      t1 = J.getInterceptor$asx(wrap);
      if (wrapMap.containsKey$1(wrapMap, t1.$index(wrap, 0)))
        result.wrapS = wrapMap.$index(wrapMap, t1.$index(wrap, 0));
      if (wrapMap.containsKey$1(wrapMap, t1.$index(wrap, 1)))
        result.wrapT = wrapMap.$index(wrapMap, t1.$index(wrap, 1));
    }
    if (anisotropy === true)
      result.anisotropy = anisotropy;
    if (!isCompressed)
      this._load_image$2(result, fullPath);
    return result;
  },
  _rgb2hex$1: function(rgb) {
    var t1 = J.getInterceptor$asx(rgb);
    return (J.toInt$0$n(J.$mul$n(t1.$index(rgb, 0), 255)) << 16 >>> 0) + (J.toInt$0$n(J.$mul$n(t1.$index(rgb, 1), 255)) << 8 >>> 0) + J.toInt$0$n(t1.$index(rgb, 2)) * 255;
  },
  _createMaterial$2: function(m, texturePath) {
    var t1, shading, mtype, blendmodes, blending, t2, transparent, depthTest, depthWrite, visible, wireframe, vertexColors, color, specular, ambient, opacity, shininess, map, lightMap, bumpMap, normalMap, specularMap, bumpScale, shader, uniforms, t3, t4, t5, material, t6, t7;
    t1 = J.getInterceptor$x(m);
    if (t1.containsKey$1(m, "shading") === true) {
      shading = J.toLowerCase$0$s(t1.$index(m, "shading"));
      if (shading === "phong")
        mtype = "MeshPhongMaterial";
      else
        mtype = shading === "basic" ? "MeshBasicMaterial" : "MeshLambertMaterial";
    } else
      mtype = "MeshLambertMaterial";
    if (t1.containsKey$1(m, "blending") === true) {
      blendmodes = H.fillLiteralMap(["NoBlending", 0, "NormalBlending", 1, "AdditiveBlending", 2, "SubtractiveBlending", 3, "MultiplyBlending", 4, "CustomBlending", 5], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
      blending = blendmodes.$index(blendmodes, t1.$index(m, "blending"));
    } else
      blending = 1;
    if (t1.containsKey$1(m, "transparent") !== true)
      t2 = t1.containsKey$1(m, "opacity") === true && J.$lt$n(t1.$index(m, "opacity"), 1);
    else
      t2 = true;
    transparent = t2 && t1.$index(m, "transparent");
    depthTest = t1.containsKey$1(m, "depthTest") !== true || t1.$index(m, "depthTest");
    depthWrite = t1.containsKey$1(m, "depthWrite") !== true || t1.$index(m, "depthWrite");
    visible = t1.containsKey$1(m, "visible") !== true || t1.$index(m, "visible");
    if (t1.containsKey$1(m, "flipSided") === true)
      ;
    if (t1.containsKey$1(m, "doubleSided") === true)
      ;
    wireframe = t1.containsKey$1(m, "wireframe") === true && t1.$index(m, "wireframe");
    if (t1.containsKey$1(m, "vertexColors") === true)
      if (J.$eq(t1.$index(m, "vertexColors"), "face"))
        vertexColors = 1;
      else
        vertexColors = t1.$index(m, "vertexColors") !== true ? 2 : null;
    else
      vertexColors = null;
    if (t1.containsKey$1(m, "colorDiffuse") === true)
      color = this._rgb2hex$1(t1.$index(m, "colorDiffuse"));
    else
      color = t1.containsKey$1(m, "DbgColor") === true ? t1.$index(m, "DbgColor") : 15658734;
    specular = t1.containsKey$1(m, "colorSpecular") === true ? this._rgb2hex$1(t1.$index(m, "colorSpecular")) : 1118481;
    ambient = t1.containsKey$1(m, "colorAmbient") === true ? this._rgb2hex$1(t1.$index(m, "colorAmbient")) : 16777215;
    opacity = t1.containsKey$1(m, "transparency") === true ? t1.$index(m, "transparency") : 1;
    shininess = t1.containsKey$1(m, "specularCoef") === true ? t1.$index(m, "specularCoef") : null;
    map = t1.containsKey$1(m, "mapDiffuse") === true && texturePath != null ? this._create_texture$6(texturePath, t1.$index(m, "mapDiffuse"), t1.$index(m, "mapDiffuseRepeat"), t1.$index(m, "mapDiffuseOffset"), t1.$index(m, "mapDiffuseWrap"), t1.$index(m, "mapDiffuseAnisotropy")) : null;
    lightMap = t1.containsKey$1(m, "mapLight") === true && texturePath != null ? this._create_texture$6(texturePath, t1.$index(m, "mapLight"), t1.$index(m, "mapLightRepeat"), t1.$index(m, "mapLightOffset"), t1.$index(m, "mapLightWrap"), t1.$index(m, "mapLightAnisotropy")) : null;
    bumpMap = t1.containsKey$1(m, "mapBump") === true && texturePath != null ? this._create_texture$6(texturePath, t1.$index(m, "mapBump"), t1.$index(m, "mapBumpRepeat"), t1.$index(m, "mapBumpOffset"), t1.$index(m, "mapBumpWrap"), t1.$index(m, "mapBumpAnisotropy")) : null;
    normalMap = t1.containsKey$1(m, "mapNormal") === true && texturePath != null ? this._create_texture$6(texturePath, t1.$index(m, "mapNormal"), t1.$index(m, "mapNormalRepeat"), t1.$index(m, "mapNormalOffset"), t1.$index(m, "mapNormalWrap"), t1.$index(m, "mapNormalAnisotropy")) : null;
    specularMap = t1.containsKey$1(m, "mapSpecular") === true && texturePath != null ? this._create_texture$6(texturePath, t1.$index(m, "mapSpecular"), t1.$index(m, "mapSpecularRepeat"), t1.$index(m, "mapSpecularOffset"), t1.$index(m, "mapSpecularWrap"), t1.$index(m, "mapSpecularAnisotropy")) : null;
    bumpScale = t1.containsKey$1(m, "mapBumpScale") === true ? t1.$index(m, "mapBumpScale") : null;
    if (t1.containsKey$1(m, "mapNormal") === true) {
      t2 = $.get$lib();
      shader = t2.$index(t2, "normal");
      uniforms = S.UniformsUtils_clone(shader.get$uniforms());
      J.set$value$x(uniforms.$index(uniforms, "tNormal"), normalMap);
      if (t1.containsKey$1(m, "mapNormalFactor") === true)
        J.get$value$x(uniforms.$index(uniforms, "uNormalScale")).setValues$2(t1.$index(m, "mapNormalFactor"), t1.$index(m, "mapNormalFactor"));
      if (map != null) {
        J.set$value$x(uniforms.$index(uniforms, "tDiffuse"), map);
        J.set$value$x(uniforms.$index(uniforms, "enableDiffuse"), true);
      }
      if (specularMap != null) {
        J.set$value$x(uniforms.$index(uniforms, "tSpecular"), specularMap);
        J.set$value$x(uniforms.$index(uniforms, "enableSpecular"), true);
      }
      if (lightMap != null) {
        J.set$value$x(uniforms.$index(uniforms, "tAO"), lightMap);
        J.set$value$x(uniforms.$index(uniforms, "enableAO"), true);
      }
      J.get$value$x(uniforms.$index(uniforms, "uDiffuseColor")).setHex$1(color);
      J.get$value$x(uniforms.$index(uniforms, "uSpecularColor")).setHex$1(specular);
      J.get$value$x(uniforms.$index(uniforms, "uAmbientColor")).setHex$1(ambient);
      J.set$value$x(uniforms.$index(uniforms, "uShininess"), shininess);
      if (opacity != null)
        J.set$value$x(uniforms.$index(uniforms, "uOpacity"), opacity);
      t2 = shader.get$fragmentShader();
      t3 = shader.get$vertexShader();
      t4 = H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
      t5 = $.MaterialCount;
      $.MaterialCount = t5 + 1;
      material = new S.ShaderMaterial(t2, t3, null, 2, false, 1, false, false, false, false, 0, true, null, t4, "", t5, 0, 1, 1, 204, 205, 100, 0, false, 0, 0, false, true, true, false, true, true, null);
      ...
      switch (mtype) {
        case "MeshLambertMaterial":
          ...
          t6 = $.MaterialCount;
          $.MaterialCount = t6 + 1;
          material = new S.MeshLambertMaterial(null, t2, t3, t4, false, t5, map, lightMap, specularMap, null, 0, 1, 0.98, 2, wireframe, 1, "round", "round", false, false, false, vertexColors, true, "", t6, 0, opacity, blending, 204, 205, 100, 0, false, 0, 0, transparent, depthTest, depthWrite, false, visible, true, null);
          break;
        case "MeshPhongMaterial":
          ...
          t7 = $.MaterialCount;
          $.MaterialCount = t7 + 1;
          material = new S.MeshPhongMaterial(t2, t3, t4, t5, shininess, false, false, false, t6, map, lightMap, bumpMap, bumpScale, null, null, specularMap, null, 0, 1, 0.98, 2, wireframe, 1, "round", "round", false, false, false, vertexColors, true, "", t7, 0, opacity, blending, 204, 205, 100, 0, false, 0, 0, transparent, depthTest, depthWrite, false, visible, true, null);
        ...
        case "MeshBasicMaterial":
          ...
          t3 = $.MaterialCount;
          $.MaterialCount = t3 + 1;
          material = new S.MeshBasicMaterial(t2, map, lightMap, specularMap, null, 0, 1, 0.98, 2, wireframe, 1, "round", "round", false, false, vertexColors, true, "", t3, 0, opacity, blending, 204, 205, 100, 0, false, 0, 0, transparent, depthTest, depthWrite, false, visible, true, null);
          break;
        default:
          H.printToConsole("Unknow material type!");
          material = null;
      }
    if (t1.containsKey$1(m, "DbgName") === true)
      material.name = t1.$index(m, "DbgName");
    return material;
  },
...
Loader_closure0: {"": "Closure;",
  call$1: function(data) {
  },
  $is_args1: true
},

Loader_closure1: {"": "Closure;",
  call$0: function() {
  }
},

Loader__load_image_closure: {"": "Closure;this_0,where_1,image_2",
  call$1: function(evt) {
    var t1, t2, t3, t4, t5, width, height;
    t1 = this.this_0;
    t2 = this.image_2;
    t3 = J.getInterceptor$x(t2);
    t4 = !t1._is_pow2$1(t3.get$width(t2)) || !t1._is_pow2$1(t3.get$height(t2));
    t5 = this.where_1;
    if (t4) {
      width = t1._nearest_pow2$1(t3.get$width(t2));
      height = t1._nearest_pow2$1(t3.get$height(t2));
      J.set$width$x(t5.image, width);
      J.set$height$x(t5.image, height);
      J.drawImage$5$x(J.getContext$1$x(t5.image, "2d"), t1, 0, 0, width, height);
    } else
      t5.image = t1;
    this.where_1.needsUpdate = true;
  },
  $is_args1: true
},
...
Material: {"": "Object;id*,side<,blending<,blendSrc<,blendDst<,blendEquation>,polygonOffset>,polygonOffsetFactor<,polygonOffsetUnits<,transparent<,depthTest<,depthWrite<,visible>,needsUpdate@,__data<",
  get$_data: function() {
    if (this.__data == null)
      this.__data = H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
    return this.__data;
...
Mesh: {"": "Object3D;geometry<,material<,morphTargetBase<,morphTargetForcedOrder,morphTargetInfluences,_morphTargetDictionary,id,name,properties,parent,children,up,position,rotation,scale,eulerOrder,_dynamic,doubleSided,flipSided,rotationAutoUpdate,renderDepth,matrix,matrixWorld,matrixRotationWorld,matrixAutoUpdate,matrixWorldNeedsUpdate,quaternion,useQuaternion,boundRadius,boundRadiusScale,visible,castShadow,receiveShadow,frustumCulled,_vector,customDepthMaterial,__data",
  Mesh$2: function(geometry, material) {
    ...
    if (t1 != null) {
      ...
      if (t1.morphTargets.length !== 0) {
        this.morphTargetBase = -1;
        this.morphTargetForcedOrder = [];
        this.morphTargetInfluences = [];
        this._morphTargetDictionary = H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
        for (m = 0; m < t1.morphTargets.length; ++m) {
          this.morphTargetInfluences.push(0);
          t2 = this._morphTargetDictionary;
          t3 = t1.morphTargets;
          if (m < 0 || m >= t3.length)
            throw H.ioore(t3, m);
          t2.$indexSet(t2, t3[m].name, m);
        }
      }
    }
  },
...
WebGLRenderer: {"": "Object;canvas,_gl,precision,_clearColor,_clearAlpha,devicePixelRatio,alpha,premultipliedAlpha,antialias,stencil,preserveDrawingBuffer,autoClear,autoClearColor,autoClearDepth,autoClearStencil,sortObjects,autoUpdateObjects,autoUpdateScene,gammaInput,gammaOutput,physicallyBasedShading,shadowMapEnabled,shadowMapAutoUpdate,shadowMapDebug,shadowMapCascade,shadowMapType,shadowMapCullFrontFaces,maxMorphTargets,maxMorphNormals,autoScaleCubemaps,renderPluginsPre,renderPluginsPost,info,_programs,_programs_counter,_currentProgram,_currentFramebuffer,_currentMaterialId,_currentGeometryGroupHash,_currentCamera,_geometryGroupCounter,_usedTextureUnits,_enabledAttributes,_oldDoubleSided,_oldFlipSided,_oldBlending,_oldBlendEquation,_oldBlendSrc,_oldBlendDst,_oldDepthTest,_oldDepthWrite,_oldPolygonOffset,_oldPolygonOffsetFactor,_oldPolygonOffsetUnits,_oldLineWidth,_viewportX,_viewportY,_viewportWidth,_viewportHeight,_currentWidth,_currentHeight,_frustum,_projScreenMatrix,_projScreenMatrixPS,_vector3,_direction,_lights,_lightsNeedUpdate,_glExtensionTextureFloat,_glExtensionStandardDerivatives,_glExtensionTextureFilterAnisotropic,_glExtensionCompressedTextureS3TC,maxAnisotropy,supportsVertexTextures,supportsBoneTextures,shadowMapPlugin,maxTextures,maxVertexTextures,maxTextureSize,maxCubemapSize",
  ...
  setViewport$4: function(x, y, width, height) {
    this._viewportX = x;
    this._viewportY = y;
    this._viewportWidth = !J.$eq(width, -1) ? width : J.get$width$x(this.canvas);
    this._viewportHeight = !J.$eq(height, -1) ? height : J.get$height$x(this.canvas);
  ...
  initMeshBuffers$2: function(geometryGroup, object) {
    ...
    t1 = object.object;
    t2 = !t1.$isMesh;
    geometry = !!t1.$isMesh || !!t1.$isLine ? t1.get$geometry() : null;
    ...
    if (uvType) {
      if (geometry.get$faceUvs().length > 0 || geometry.faceVertexUvs.length > 0) {
      ...
      if (geometry.faceUvs.length > 1 || geometry.faceVertexUvs.length > 1) {
        t6 = new Float32Array(nvertices * 2);
        t6.$dartCachedLength = t6.length;
        geometryGroup.__uv2Array = t6;
      }
    }
    if ((!!t1.$isMesh || !!t1.$isLine ? t1.get$geometry() : null).get$skinWeights().length !== 0)
      t1 = (!!t1.$isMesh || !!t1.$isLine ? t1.get$geometry() : null).get$skinIndices().length !== 0;
    else
      t1 = false;
    if (t1) {
      t1 = nvertices * 4;
      t2 = new Float32Array(t1);
      t2.$dartCachedLength = t2.length;
      geometryGroup.__skinIndexArray = t2;
      t1 = new Float32Array(t1);
      t1.$dartCachedLength = t1.length;
      geometryGroup.__skinWeightArray = t1;
    }
    t1 = (t3 * 1 + t4 * 2) * 3;
    ...
    if (J.get$map$ax(material) == null)
      if (material.get$lightMap() == null)
        if (material.get$bumpMap() == null) {
          ...
          t1 = material.get$specularMap() != null || material.get$isShaderMaterial();
        } else
          t1 = true;
      else
        t1 = true;
  ...
  setLineBuffers$2: function(geometry, hint) {
    ...
    lineDistances = t1.get$lineDistances();
    ...
    t2 = t1.get$__data();
    dirtyVertices = t2.containsKey$1(t2, "verticesNeedUpdate") && t1.$index(t1, "verticesNeedUpdate");
    t2 = t1.__data;
    dirtyColors = t2.containsKey$1(t2, "colorsNeedUpdate") && t1.$index(t1, "colorsNeedUpdate");
    t2 = t1.__data;
    dirtyLineDistances = t2.containsKey$1(t2, "lineDistancesNeedUpdate") && t1.$index(t1, "lineDistancesNeedUpdate");
  ...
  setMeshBuffers$5: function(geometryGroup, object, hint, dispose, material) {
    var normalType, vertexColorType, uvType, needsSmoothNormals, vertexArray, uvArray, uv2Array, normalArray, tangentArray, colorArray, skinIndexArray, skinWeightArray, morphTargetsArrays, morphNormalsArrays, customAttributes, faceArray, lineArray, t1, t2, geometry, dirtyVertices, dirtyElements, dirtyUvs, dirtyNormals, dirtyTangents, dirtyColors, dirtyMorphTargets, vertices, chunk_faces3, chunk_faces4, obj_faces, obj_uvs, obj_uvs2, obj_skinIndices, obj_skinWeights, morphTargets, morphNormals, fl, f, face, v1, v2, v3, offset, t3, t4, t5, v4, vkl, n1, n2, n3, n4, vk, vka, nka, chf, faceVertexNormals, offset_morphTarget, t6, t7, t8, t9, t10, t11, t12, t13, t14, offset_skin, sw1, sw2, sw3, si1, si2, si3, t15, sw4, t16, t17, t18, si4, t19, offset_color, vertexColors, faceColor, c1, c2, c3, c4, offset_tangent, vertexTangents, i, vn, offset_normal, vertexNormals, faceNormal, uvi, offset_uv, fi, uv, uv2i, offset_uv2, uv2, vertexIndex, offset_face, offset_line, il, offset_custom, offset_customSrc, value;
    ...
    t1 = object.object;
    t2 = !t1.$isMesh;
    if ((!!t1.$isMesh || !!t1.$isLine ? t1.get$geometry() : null) != null) {
      t1 = !!t1.$isMesh || !!t1.$isLine ? t1.get$geometry() : null;
      t2 = J.getInterceptor$asx(t1);
      if (t2.$index(t1, "__webglBuffer") == null)
        t2.$indexSet(t1, "__webglBuffer", new S.WebGLGeometry(t2.get$id(t1), null, null, null, 0, 0, 0, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, t1));
      geometry = t2.$index(t1, "__webglBuffer");
    } else
      geometry = null;
    dirtyVertices = geometry.get$verticesNeedUpdate();
    t1 = geometry._geometry;
    t2 = t1.get$__data();
    dirtyElements = t2.containsKey$1(t2, "elementsNeedUpdate") && t1.$index(t1, "elementsNeedUpdate");
    t2 = t1.get$__data();
    dirtyUvs = t2.containsKey$1(t2, "uvsNeedUpdate") && t1.$index(t1, "uvsNeedUpdate");
    t2 = t1.get$__data();
    dirtyNormals = t2.containsKey$1(t2, "normalsNeedUpdate") && t1.$index(t1, "normalsNeedUpdate");
    t2 = t1.get$__data();
    dirtyTangents = t2.containsKey$1(t2, "tangentsNeedUpdate") && t1.$index(t1, "tangentsNeedUpdate");
    t2 = t1.get$__data();
    dirtyColors = t2.containsKey$1(t2, "colorsNeedUpdate") && t1.$index(t1, "colorsNeedUpdate");
    t2 = t1.get$__data();
    dirtyMorphTargets = t2.containsKey$1(t2, "morphTargetsNeedUpdate") && t1.$index(t1, "morphTargetsNeedUpdate");
    vertices = geometry.get$vertices();
    chunk_faces3 = geometryGroup.faces3;
    chunk_faces4 = geometryGroup.faces4;
    obj_faces = t1.get$faces();
    if (t1.faceVertexUvs.length === 0)
      obj_uvs = [];
    else {
      t2 = t1.faceVertexUvs;
      if (0 >= t2.length)
    ...
    if (dirtyVertices === true) {
      ...
      for (t2 = J.getInterceptor$asx(vertices), f = 0, face = null, v1 = null, v2 = null, v3 = null, offset = 0; f < fl; ++f) {
        ...
        face = obj_faces[t3];
        v1 = t2.$index(vertices, face.get$a(face));
        t3 = face.get$b(face);
    ...
    if (dirtyMorphTargets === true) {
      ...
      for (n1 = null, n2 = null, n3 = null, n4 = null, vk = 0, vka = null, nka = null, chf = null, faceVertexNormals = null; vk < vkl; ++vk) {
        ...
        for (f = 0, offset_morphTarget = 0; f < fl; ++f) {
          ...
          v1 = J.$index$asx(morphTargets[vk].get$vertices(), face.get$a(face));
          if (vk >= morphTargets.length)
            throw H.ioore(morphTargets, vk);
          v2 = J.$index$asx(morphTargets[vk].get$vertices(), face.get$b(face));
          if (vk >= morphTargets.length)
            throw H.ioore(morphTargets, vk);
          v3 = J.$index$asx(morphTargets[vk].get$vertices(), face.get$c(face));
          ...
          t5 = J.getInterceptor$x(v2);
          t6 = t5.get$x(v2);
          if (t2 >= C.JS_CONST_ZYJ(vka))
            throw H.ioore(vka, t2);
          vka[t2] = t6;
          ...
          vka[t7] = t5;
          t5 = offset_morphTarget + 6;
          t8 = J.getInterceptor$x(v3);
          t9 = t8.get$x(v3);
          ...
          if (material.get$morphNormals() === true) {
            if (needsSmoothNormals) {
              if (vk < 0 || vk >= morphNormals.length)
                throw H.ioore(morphNormals, vk);
              t8 = morphNormals[vk].get$vertexNormals();
              if (chf < 0 || chf >= t8.length)
                throw H.ioore(t8, chf);
              faceVertexNormals = t8[chf];
              t8 = J.getInterceptor$x(faceVertexNormals);
              n1 = t8.get$a(faceVertexNormals);
              n2 = t8.get$b(faceVertexNormals);
              n3 = t8.get$c(faceVertexNormals);
            } else {
              if (vk < 0 || vk >= morphNormals.length)
                throw H.ioore(morphNormals, vk);
              t8 = morphNormals[vk].get$faceNormals();
              n1 = t8.$index(t8, chf);
              n3 = n1;
              n2 = n3;
            }
            ...
            t8 = J.getInterceptor$x(n1);
            t11 = t8.get$x(n1);
            if (offset_morphTarget >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, offset_morphTarget);
            nka[offset_morphTarget] = t11;
            t11 = t8.get$y(n1);
            if (t3 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t3);
            nka[t3] = t11;
            t8 = t8.get$z(n1);
            if (t4 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t4);
            nka[t4] = t8;
            t8 = J.getInterceptor$x(n2);
            t4 = t8.get$x(n2);
            if (t2 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t2);
            nka[t2] = t4;
            t4 = t8.get$y(n2);
            if (t6 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t6);
            nka[t6] = t4;
            t8 = t8.get$z(n2);
            if (t7 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t7);
            nka[t7] = t8;
            t8 = J.getInterceptor$x(n3);
            t7 = t8.get$x(n3);
            if (t5 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t5);
            nka[t5] = t7;
            t7 = t8.get$y(n3);
            if (t9 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t9);
            nka[t9] = t7;
            t8 = t8.get$z(n3);
            if (t10 >= C.JS_CONST_ZYJ(nka))
        ...
        for (f = 0; f < fl; ++f) {
          ...
          if (vk >= morphTargets.length)
            throw H.ioore(morphTargets, vk);
          v4 = J.$index$asx(morphTargets[vk].get$vertices(), face.get$d(face));
          if (vk >= morphTargetsArrays.length)
            throw H.ioore(morphTargetsArrays, vk);
          vka = morphTargetsArrays[vk];
          t2 = J.getInterceptor$x(v1);
          ...
          if (t10 >= C.JS_CONST_ZYJ(vka))
            throw H.ioore(vka, t10);
          vka[t10] = t8;
          t8 = offset_morphTarget + 9;
          t11 = J.getInterceptor$x(v4);
          t12 = t11.get$x(v4);
          ...
          if (material.get$morphNormals() === true) {
            if (needsSmoothNormals) {
              if (vk < 0 || vk >= morphNormals.length)
                throw H.ioore(morphNormals, vk);
              t11 = morphNormals[vk].get$vertexNormals();
              if (chf < 0 || chf >= t11.length)
                throw H.ioore(t11, chf);
              faceVertexNormals = t11[chf];
              t11 = J.getInterceptor$x(faceVertexNormals);
              n1 = t11.get$a(faceVertexNormals);
              n2 = t11.get$b(faceVertexNormals);
              n3 = t11.get$c(faceVertexNormals);
              n4 = t11.get$d(faceVertexNormals);
            } else {
              if (vk < 0 || vk >= morphNormals.length)
                throw H.ioore(morphNormals, vk);
              t11 = morphNormals[vk].get$faceNormals();
              n1 = t11.$index(t11, chf);
              n4 = n1;
              n3 = n4;
              n2 = n3;
            }
            if (vk >= morphNormalsArrays.length)
              throw H.ioore(morphNormalsArrays, vk);
            nka = morphNormalsArrays[vk];
            t11 = J.getInterceptor$x(n1);
            t14 = t11.get$x(n1);
            if (offset_morphTarget >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, offset_morphTarget);
            nka[offset_morphTarget] = t14;
            t14 = t11.get$y(n1);
            if (t3 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t3);
            nka[t3] = t14;
            ...
            if (t10 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t10);
            nka[t10] = t11;
            t11 = J.getInterceptor$x(n4);
            t10 = t11.get$x(n4);
            if (t8 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t8);
            nka[t8] = t10;
            t10 = t11.get$y(n4);
            if (t12 >= C.JS_CONST_ZYJ(nka))
              throw H.ioore(nka, t12);
            nka[t12] = t10;
            t11 = t11.get$z(n4);
            if (t13 >= C.JS_CONST_ZYJ(nka))
    ...
    if (obj_skinWeights.length !== 0) {
      ...
      for (f = 0, offset_skin = 0; f < fl; ++f) {
        ...
        if (t2 < 0 || t2 >= obj_faces.length)
          throw H.ioore(obj_faces, t2);
        face = obj_faces[t2];
        t2 = face.get$a(face);
        if (t2 >>> 0 !== t2 || t2 >= obj_skinWeights.length)
          throw H.ioore(obj_skinWeights, t2);
        sw1 = obj_skinWeights[t2];
        t2 = face.get$b(face);
        if (t2 >>> 0 !== t2 || t2 >= obj_skinWeights.length)
          throw H.ioore(obj_skinWeights, t2);
        sw2 = obj_skinWeights[t2];
        t2 = face.get$c(face);
        if (t2 >>> 0 !== t2 || t2 >= obj_skinWeights.length)
          throw H.ioore(obj_skinWeights, t2);
        sw3 = obj_skinWeights[t2];
        t2 = sw1.storage;
        t3 = t2[0];
        t4 = C.JS_CONST_ZYJ(skinWeightArray);
        if (offset_skin >= t4)
          throw H.ioore(skinWeightArray, offset_skin);
        skinWeightArray[offset_skin] = t3;
        t3 = offset_skin + 1;
        t5 = t2[1];
        if (t3 >= t4)
          throw H.ioore(skinWeightArray, t3);
        skinWeightArray[t3] = t5;
        t5 = offset_skin + 2;
        t6 = t2[2];
        if (t5 >= t4)
          throw H.ioore(skinWeightArray, t5);
        skinWeightArray[t5] = t6;
        t6 = offset_skin + 3;
        t2 = t2[3];
        if (t6 >= t4)
          throw H.ioore(skinWeightArray, t6);
        skinWeightArray[t6] = t2;
        t2 = offset_skin + 4;
        t7 = sw2.storage;
        t8 = t7[0];
        if (t2 >= t4)
          throw H.ioore(skinWeightArray, t2);
        skinWeightArray[t2] = t8;
        t8 = offset_skin + 5;
        t9 = t7[1];
        if (t8 >= t4)
          throw H.ioore(skinWeightArray, t8);
        skinWeightArray[t8] = t9;
        t9 = offset_skin + 6;
        t10 = t7[2];
        if (t9 >= t4)
          throw H.ioore(skinWeightArray, t9);
        skinWeightArray[t9] = t10;
        t10 = offset_skin + 7;
        t7 = t7[3];
        if (t10 >= t4)
          throw H.ioore(skinWeightArray, t10);
        skinWeightArray[t10] = t7;
        t7 = offset_skin + 8;
        t11 = sw3.storage;
        t12 = t11[0];
        if (t7 >= t4)
          throw H.ioore(skinWeightArray, t7);
        skinWeightArray[t7] = t12;
        t12 = offset_skin + 9;
        t13 = t11[1];
        if (t12 >= t4)
          throw H.ioore(skinWeightArray, t12);
        skinWeightArray[t12] = t13;
        t13 = offset_skin + 10;
        t14 = t11[2];
        if (t13 >= t4)
          throw H.ioore(skinWeightArray, t13);
        skinWeightArray[t13] = t14;
        t14 = offset_skin + 11;
        t11 = t11[3];
        if (t14 >= t4)
          throw H.ioore(skinWeightArray, t14);
        skinWeightArray[t14] = t11;
        t11 = face.get$a(face);
        if (t11 >>> 0 !== t11 || t11 >= obj_skinIndices.length)
          throw H.ioore(obj_skinIndices, t11);
        si1 = obj_skinIndices[t11];
        t11 = face.get$b(face);
        if (t11 >>> 0 !== t11 || t11 >= obj_skinIndices.length)
          throw H.ioore(obj_skinIndices, t11);
        si2 = obj_skinIndices[t11];
        t11 = face.get$c(face);
        if (t11 >>> 0 !== t11 || t11 >= obj_skinIndices.length)
          throw H.ioore(obj_skinIndices, t11);
        si3 = obj_skinIndices[t11];
        t11 = si1.storage;
        t4 = t11[0];
        t15 = C.JS_CONST_ZYJ(skinIndexArray);
        if (offset_skin >= t15)
          throw H.ioore(skinIndexArray, offset_skin);
        skinIndexArray[offset_skin] = t4;
        t4 = t11[1];
        if (t3 >= t15)
          throw H.ioore(skinIndexArray, t3);
        skinIndexArray[t3] = t4;
        t4 = t11[2];
        if (t5 >= t15)
          throw H.ioore(skinIndexArray, t5);
        skinIndexArray[t5] = t4;
        t11 = t11[3];
        if (t6 >= t15)
          throw H.ioore(skinIndexArray, t6);
        skinIndexArray[t6] = t11;
        t11 = si2.storage;
        t6 = t11[0];
        if (t2 >= t15)
          throw H.ioore(skinIndexArray, t2);
        skinIndexArray[t2] = t6;
        t6 = t11[1];
        if (t8 >= t15)
          throw H.ioore(skinIndexArray, t8);
        skinIndexArray[t8] = t6;
        t6 = t11[2];
        if (t9 >= t15)
          throw H.ioore(skinIndexArray, t9);
        skinIndexArray[t9] = t6;
        t11 = t11[3];
        if (t10 >= t15)
          throw H.ioore(skinIndexArray, t10);
        skinIndexArray[t10] = t11;
        t11 = si3.storage;
        t10 = t11[0];
        if (t7 >= t15)
          throw H.ioore(skinIndexArray, t7);
        skinIndexArray[t7] = t10;
        t10 = t11[1];
        if (t12 >= t15)
          throw H.ioore(skinIndexArray, t12);
        skinIndexArray[t12] = t10;
        t10 = t11[2];
        if (t13 >= t15)
          throw H.ioore(skinIndexArray, t13);
        skinIndexArray[t13] = t10;
        t11 = t11[3];
        if (t14 >= t15)
          throw H.ioore(skinIndexArray, t14);
        skinIndexArray[t14] = t11;
        offset_skin += 12;
      }
      fl = chunk_faces4.length;
      for (f = 0; f < fl; ++f) {
        ...
        t16 = t15[0];
        if (t11 >= t4)
          throw H.ioore(skinWeightArray, t11);
        skinWeightArray[t11] = t16;
        t16 = offset_skin + 13;
        t17 = t15[1];
        if (t16 >= t4)
          throw H.ioore(skinWeightArray, t16);
        skinWeightArray[t16] = t17;
        t17 = offset_skin + 14;
        t18 = t15[2];
        if (t17 >= t4)
          throw H.ioore(skinWeightArray, t17);
        skinWeightArray[t17] = t18;
        t18 = offset_skin + 15;
        t15 = t15[3];
        if (t18 >= t4)
          throw H.ioore(skinWeightArray, t18);
        skinWeightArray[t18] = t15;
        t15 = face.get$a(face);
        if (t15 >>> 0 !== t15 || t15 >= obj_skinIndices.length)
          throw H.ioore(obj_skinIndices, t15);
        ...
        if (t15 >>> 0 !== t15 || t15 >= obj_skinIndices.length)
          throw H.ioore(obj_skinIndices, t15);
        si3 = obj_skinIndices[t15];
        t15 = face.get$d(face);
        if (t15 >>> 0 !== t15 || t15 >= obj_skinIndices.length)
          throw H.ioore(obj_skinIndices, t15);
        si4 = obj_skinIndices[t15];
        t15 = si1.storage;
        t4 = t15[0];
        t19 = C.JS_CONST_ZYJ(skinIndexArray);
        if (offset_skin >= t19)
          throw H.ioore(skinIndexArray, offset_skin);
        skinIndexArray[offset_skin] = t4;
        t4 = t15[1];
        ...
        if (t14 >= t19)
          throw H.ioore(skinIndexArray, t14);
        skinIndexArray[t14] = t15;
        t15 = si4.storage;
        t14 = t15[0];
        if (t11 >= t19)
          throw H.ioore(skinIndexArray, t11);
        skinIndexArray[t11] = t14;
        t14 = t15[1];
        if (t16 >= t19)
          throw H.ioore(skinIndexArray, t16);
        skinIndexArray[t16] = t14;
        t14 = t15[2];
        if (t17 >= t19)
          throw H.ioore(skinIndexArray, t17);
        skinIndexArray[t17] = t14;
        t15 = t15[3];
        if (t18 >= t19)
          throw H.ioore(skinIndexArray, t18);
        skinIndexArray[t18] = t15;
        offset_skin += 16;
      }
      if (offset_skin > 0) {
    ...
    if (t1 && obj_uvs2 != null && uvType) {
      ...
      for (f = 0, uv2i = null, offset_uv2 = 0; f < fl; ++f) {
        ...
        if (fi < 0 || fi >= obj_uvs2.length)
          throw H.ioore(obj_uvs2, fi);
        uv2 = obj_uvs2[fi];
        if (uv2 == null)
          continue;
  ...
  setupMorphTargets$3: function(material, geometryGroup, object) {
    var attributes, t1, t2, t3, t4, activeInfluenceIndices, influences, il, i, influence, influenceIndex, m, t5, t6;
    ...
    for (t1 = material._material, t2 = !t1.$isMeshLambertMaterial, t3 = !t1.$isMeshPhongMaterial, t4 = !!t1.$isShaderMaterial, influenceIndex = null, m = 0; m < material.numSupportedMorphTargets;) {
      t5 = activeInfluenceIndices.length;
      if (m < t5) {
        t6 = activeInfluenceIndices[m];
        t6 = t6 != null && t6.length !== 0;
      } else
        t6 = false;
      if (t6) {
        if (m >= t5)
        ...
        influenceIndex = t5[0];
        t5 = this._gl;
        t6 = geometryGroup.get$__webglMorphTargetsBuffers();
        if (influenceIndex >>> 0 !== influenceIndex || influenceIndex >= t6.length)
          throw H.ioore(t6, influenceIndex);
        J.bindBuffer$2$x(t5, 34962, t6[influenceIndex]);
        ...
        if ((!!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || t4) && t1.get$morphNormals()) {
          t5 = this._gl;
          t6 = geometryGroup.__webglMorphNormalsBuffers;
          if (influenceIndex >= t6.length)
            throw H.ioore(t6, influenceIndex);
          J.bindBuffer$2$x(t5, 34962, t6[influenceIndex]);
        ...
        t5 = object.__webglMorphTargetInfluences;
        if (influenceIndex >= influences.length)
          throw H.ioore(influences, influenceIndex);
        t6 = influences[influenceIndex];
        if (m >= C.JS_CONST_ZYJ(t5))
          throw H.ioore(t5, m);
        t5[m] = t6;
      } else {
        t5 = object.__webglMorphTargetInfluences;
  ...
  _render$4$forceClear$renderTarget: function(scene, c, forceClear, renderTarget) {
    ...
    for (; i < il; ++i) {
      ...
      if (object.get$visible(object))
        if (!(!!object.$isMesh || false) || !object.frustumCulled || t1.contains$1(t1, object)) {
  ...
  renderObjects$8: function(renderList, reverse, materialType, camera, lights, fog, useBlending, overrideMaterial) {
    ...
    for (t1 = J.getInterceptor$asx(renderList), t2 = materialType === "opaque", i = start, object = null, buffer = null, material = null; i !== end; i += delta) {
      ...
      if (webglObject.get$render() === true) {
        ...
        if (!J.$eq(this._oldDepthWrite, t3)) {
  ...
  sortFacesByMaterial$2: function(geometry, material) {
    var hash_map, t1, numMorphTargets, numMorphNormals, fl, f, face, groupHash, t2, vertices, t3, t4;
    hash_map = H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
    t1 = geometry._geometry;
    numMorphTargets = J.get$length$asx(t1.get$morphTargets());
    numMorphNormals = J.get$length$asx(t1.get$morphNormals());
    geometry.geometryGroups = H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
    fl = J.get$length$asx(t1.get$faces());
    for (f = 0; f < fl; ++f) {
      face = J.$index$asx(t1.get$faces(), f);
  ...
  updateObject$1: function(webglobject) {
    var object, t1, t2, geometry, il, material, i, t3, geometryGroup, customAttributesDirty;
    object = webglobject.get$object();
    ...
    if ((!!t1.$isMesh || !!t1.$isLine ? t1.get$geometry() : null) != null) {
      ...
      geometry = null;
    if (!!object.$isMesh) {
      for (il = geometry.get$geometryGroupsList().length, t1 = geometry._geometry, t2 = J.getInterceptor$asx(t1), material = null, i = 0; i < il; ++i) {
        t3 = geometry.geometryGroupsList;
        if (i >= t3.length)
          throw H.ioore(t3, i);
        geometryGroup = t3[i];
        material = this.getBufferMaterial$2(webglobject, geometryGroup);
        t3 = t1.get$__data();
        if ((t3.containsKey$1(t3, "buffersNeedUpdate") && t2.$index(t1, "buffersNeedUpdate")) === true)
        ...
        t3 = t1.get$__data();
        if ((t3.containsKey$1(t3, "verticesNeedUpdate") && t2.$index(t1, "verticesNeedUpdate")) !== true) {
          t3 = t1.get$__data();
          if ((t3.containsKey$1(t3, "morphTargetsNeedUpdate") && t2.$index(t1, "morphTargetsNeedUpdate")) !== true) {
            t3 = t1.get$__data();
            if ((t3.containsKey$1(t3, "elementsNeedUpdate") && t2.$index(t1, "elementsNeedUpdate")) !== true) {
              t3 = t1.get$__data();
              if ((t3.containsKey$1(t3, "uvsNeedUpdate") && t2.$index(t1, "uvsNeedUpdate")) !== true) {
                t3 = t1.get$__data();
                if ((t3.containsKey$1(t3, "normalsNeedUpdate") && t2.$index(t1, "normalsNeedUpdate")) !== true) {
                  t3 = t1.get$__data();
                  if ((t3.containsKey$1(t3, "colorsNeedUpdate") && t2.$index(t1, "colorsNeedUpdate")) !== true) {
                    t3 = t1.get$__data();
                    t3 = (t3.containsKey$1(t3, "tangentsNeedUpdate") && t2.$index(t1, "tangentsNeedUpdate")) === true || customAttributesDirty;
                  } else
                    t3 = true;
                } else
                  t3 = true;
          ...
          t3 = true;
        if (t3)
          this.setMeshBuffers$5(geometryGroup, webglobject, 35048, t1.get$isDynamic() !== true, material);
      }
      t2.$indexSet(t1, "verticesNeedUpdate", false);
      t2.$indexSet(t1, "morphTargetsNeedUpdate", false);
      t2.$indexSet(t1, "elementsNeedUpdate", false);
      t2.$indexSet(t1, "uvsNeedUpdate", false);
      t2.$indexSet(t1, "normalsNeedUpdate", false);
      t2.$indexSet(t1, "colorsNeedUpdate", false);
      t2.$indexSet(t1, "tangentsNeedUpdate", false);
      t2.$indexSet(t1, "buffersNeedUpdate", false);
      ...
      if (geometry.get$verticesNeedUpdate() !== true) {
        ...
        t3 = t2.get$__data();
        if ((t3.containsKey$1(t3, "colorsNeedUpdate") && t2.$index(t2, "colorsNeedUpdate")) !== true) {
          t3 = t2.get$__data();
          t2 = (t3.containsKey$1(t3, "lineDistancesNeedUpdate") && t2.$index(t2, "lineDistancesNeedUpdate")) === true || customAttributesDirty;
        } else
          t2 = true;
      } else
        t2 = true;
      ...
      t3.$indexSet(t2, "verticesNeedUpdate", false);
      t3.$indexSet(t2, "colorsNeedUpdate", false);
      t3.$indexSet(t2, "lineDistancesNeedUpdate", false);
      if (t1.get$attributes(material) != null)
        this.clearCustomAttributes$1(material);
    }
  },
  ...
  initMaterial$4: function(material, lights, fog, webglobject) {
    var object, t1, shaderID, maxLightCount, maxShadows, maxBones, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19, t20, t21, t22, t23, t24, t25, t26, t27, t28, t29, t30, t31, t32, t33, attributes, i;
    object = webglobject.get$object();
    t1 = material._material;
    if (!!t1.$isMeshBasicMaterial)
      shaderID = "basic";
    else if (!!t1.$isMeshLambertMaterial)
      shaderID = "lambert";
    else if (!!t1.$isMeshPhongMaterial)
      shaderID = "phong";
    ...
    t9 = !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || false ? t1.get$map(t1) : null;
    t10 = !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial ? t1.get$envMap() : null;
    t11 = !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial ? t1.get$lightMap() : null;
    t12 = !!t1.$isMeshPhongMaterial;
    t13 = t12 ? H.interceptedTypeCast(t1, "$isMeshPhongMaterial").bumpMap : null;
    t14 = t12 ? t1.get$normalMap() : null;
    t15 = !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t12 ? t1.get$specularMap() : null;
    t16 = material.get$_hasVertexColors() ? t1.get$vertexColors() : 0;
    t17 = material.get$_hasFog() && t1.get$fog();
    t18 = (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t12 || t2) && t1.get$skinning();
    if (this.supportsBoneTextures === true)
      t19 = false;
    else
      t19 = false;
    t20 = (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t12 || t2) && t1.get$morphTargets();
    t21 = !t1.$isMeshLambertMaterial;
    t22 = (!!t1.$isMeshLambertMaterial || t12 || t2) && t1.get$morphNormals();
    t23 = this.maxMorphTargets;
    t24 = this.maxMorphNormals;
    t25 = maxLightCount.$index(maxLightCount, "directional");
    t26 = maxLightCount.$index(maxLightCount, "point");
    t27 = maxLightCount.$index(maxLightCount, "spot");
    t28 = maxLightCount.$index(maxLightCount, "hemi");
    t29 = this.shadowMapEnabled && object.get$receiveShadow();
    t30 = t12 && H.interceptedTypeCast(t1, "$isMeshPhongMaterial").metal;
    t31 = t12 && H.interceptedTypeCast(t1, "$isMeshPhongMaterial").perPixel;
    t32 = (!!t1.$isMeshLambertMaterial || t12) && t1.get$wrapAround();
    t33 = t1.side;
    material.program = this.buildProgram$41$alphaTest$boneTextureHeight$boneTextureWidth$bumpMap$doubleSided$envMap$flipSided$fog$fogExp$lightMap$map$maxBones$maxDirLights$maxHemiLights$maxMorphNormals$maxMorphTargets$maxPointLights$maxShadows$maxSpotLights$metal$morphNormals$morphTargets$normalMap$perPixel$shadowMapCascade$shadowMapDebug$shadowMapEnabled$shadowMapType$sizeAttenuation$skinning$specularMap$useFog$useVertexTexture$vertexColors$wrapAround(shaderID, t3, t4, t5, t6, t7, t1.alphaTest, null, null, t13, t33 === 2, t10, t33 === 1, fog, false, t11, t9, maxBones, t25, t28, t24, t23, t26, maxShadows, t27, t30, t22, t20, t14, t31, this.shadowMapCascade, this.shadowMapDebug, t29, this.shadowMapType, false, t18, t15, t17, t19, t16, t32);
    attributes = material.program.attributes;
    if ((!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t12 || t2) && t1.get$morphTargets()) {
    ...
    if ((!!t1.$isMeshLambertMaterial || t12 || t2) && t1.get$morphNormals()) {
  ...
  setProgram$5: function(camera, lights, fog, material, object) {
    var t1, program, p_uniforms, t2, m_uniforms, t3, refreshMaterial, t4, t5, t6;
    ...
    if ((!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || t2) && t1.get$skinning()) {
      if (this.supportsBoneTextures === true)
        object.get$useVertexTexture();
      t4 = p_uniforms.get$boneGlobalMatrices();
      J.uniformMatrix4fv$3$x(this._gl, t4, false, object.get$boneMatrices());
    }
    if (refreshMaterial) {
      if (fog != null && material.get$_hasFog() && t1.get$fog())
        this.refreshUniformsFog$2(m_uniforms, fog);
      t4 = !!t1.$isMeshPhongMaterial;
      t5 = !t4;
      if (t5)
        if (!t1.$isMeshLambertMaterial)
          t6 = t2 && H.interceptedTypeCast(t1, "$isShaderMaterial").lights;
        else
          t6 = true;
      else
        t6 = true;
      ...
      if (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t4)
      ...
      if (t2)
        if (t5) {
          if (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || false)
            ;
          t4 = false;
        } else
          t4 = true;
      ...
      if (t4)
        if (p_uniforms.$index(p_uniforms, "cameraPosition") != null) {
          ...
          t4 = this._vector3.storage;
          J.uniform3f$4$x(this._gl, p_uniforms.$index(p_uniforms, "cameraPosition"), t4[0], t4[1], t4[2]);
        }
      if (t5) {
        t4 = !t1.$isMeshLambertMaterial;
        if (t4)
          if (t2)
            t1 = (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t4 || false) && t1.get$skinning();
          else
            t1 = true;
        else
          t1 = true;
  ...
  refreshUniformsCommon$2: function(uniforms, material) {
    var t1, t2, t3, t4, t5, uvScaleMap, offset, repeat, t6, t7, t8;
    t1 = material._material;
    J.set$value$x(uniforms.$index(uniforms, "opacity"), t1.opacity);
    t2 = this.gammaInput;
    if (t2)
      J.get$value$x(uniforms.$index(uniforms, "diffuse")).copyGammaToLinear$1(t1.get$color(t1));
    else
      J.set$value$x(uniforms.$index(uniforms, "diffuse"), t1.get$color(t1));
    t3 = uniforms.$index(uniforms, "map");
    t4 = !t1.$isMeshBasicMaterial;
    J.set$value$x(t3, !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || false ? t1.get$map(t1) : null);
    t3 = uniforms.$index(uniforms, "lightMap");
    J.set$value$x(t3, !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial ? t1.get$lightMap() : null);
    t3 = uniforms.$index(uniforms, "specularMap");
    J.set$value$x(t3, !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial ? t1.get$specularMap() : null);
    t3 = !!t1.$isMeshPhongMaterial;
    if ((t3 ? H.interceptedTypeCast(t1, "$isMeshPhongMaterial").bumpMap : null) != null) {
      t5 = uniforms.$index(uniforms, "bumpMap");
      J.set$value$x(t5, t3 ? H.interceptedTypeCast(t1, "$isMeshPhongMaterial").bumpMap : null);
      J.set$value$x(uniforms.$index(uniforms, "bumpScale"), material.get$bumpScale());
    }
    if (t3)
      ;
    if ((!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t3 || false ? t1.get$map(t1) : null) != null)
      uvScaleMap = !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t3 || false ? t1.get$map(t1) : null;
    else if ((!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t3 ? t1.get$specularMap() : null) != null)
      uvScaleMap = !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t3 ? t1.get$specularMap() : null;
    else {
      if (t3)
        ;
      uvScaleMap = (t3 ? H.interceptedTypeCast(t1, "$isMeshPhongMaterial").bumpMap : null) != null ? t3 ? H.interceptedTypeCast(t1, "$isMeshPhongMaterial").bumpMap : null : null;
    }
    if (uvScaleMap != null) {
      offset = J.get$offset$x(uvScaleMap);
      repeat = uvScaleMap.get$repeat();
      t5 = J.get$value$x(uniforms.$index(uniforms, "offsetRepeat"));
      t6 = offset.storage;
      t7 = t6[0];
      t6 = t6[1];
      t8 = repeat.storage;
      t5.setValues$4(t7, t6, t8[0], t8[1]);
    }
    t5 = uniforms.$index(uniforms, "envMap");
    J.set$value$x(t5, !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t3 ? t1.get$envMap() : null);
    t5 = uniforms.$index(uniforms, "flipEnvMap");
    if (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t3)
      ;
    J.set$value$x(t5, -1);
    if (t2)
    ...
    t2 = uniforms.$index(uniforms, "useRefract");
    if (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || t3)
      ;
    J.set$value$x(t2, 0);
  },
  ...
  refreshUniformsPhong$2: function(uniforms, material) {
    var t1, t2, t3;
    t1 = uniforms.$index(uniforms, "shininess");
    t2 = material._material;
    t3 = !!t2.$isMeshPhongMaterial;
    J.set$value$x(t1, t3 ? t2.get$shininess() : null);
    if (this.gammaInput) {
      J.get$value$x(uniforms.$index(uniforms, "ambient")).copyGammaToLinear$1(t2.get$ambient());
      J.get$value$x(uniforms.$index(uniforms, "emissive")).copyGammaToLinear$1(t2.get$emissive());
      t1 = J.get$value$x(uniforms.$index(uniforms, "specular"));
      t1.copyGammaToLinear$1(t3 ? t2.get$specular() : null);
    } else {
      J.set$value$x(uniforms.$index(uniforms, "ambient"), t2.get$ambient());
      J.set$value$x(uniforms.$index(uniforms, "emissive"), t2.get$emissive());
      t1 = uniforms.$index(uniforms, "specular");
      J.set$value$x(t1, t3 ? t2.get$specular() : null);
    }
    if ((!!t2.$isMeshLambertMaterial || t3) && t2.get$wrapAround())
  ...
  refreshUniformsLambert$2: function(uniforms, material) {
    ...
    t1 = material._material;
    if ((!!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial) && t1.get$wrapAround())
      J.get$value$x(uniforms.$index(uniforms, "wrapRGB")).copy$1(material.get$wrapRGB());
  },
  ...
  loadUniformsGeneric$2: function(program, uniforms) {
    ...
    for (uniform = null, value = null, type = null, texture = null, textureUnit = null, i = null, il = null, j = 0; j < jl; ++j) {
      ...
      if (type === "i")
        ...
        t1 = texture.image;
        t2 = J.getInterceptor(t1);
        t2 = typeof t1 === "object" && t1 !== null && !!t2.$isWebGLImageList;
        t1 = t2;
        if (t1 && J.get$length$asx(texture.image) === 6)
          this.setCubeTexture$2(texture, textureUnit);
        else
  ...
  setDepthTest$1: function(depthTest) {
    var t1;
    if (!J.$eq(this._oldDepthTest, depthTest)) {
      t1 = this._gl;
      if (depthTest === true)
  ...
  setDepthWrite$1: function(depthWrite) {
    if (!J.$eq(this._oldDepthWrite, depthWrite)) {
  ...
  setBlending$4: function(blending, blendEquation, blendSrc, blendDst) {
    var t1, t2, t3;
    t1 = J.getInterceptor(blending);
    if (!t1.$eq(blending, this._oldBlending)) {
      if (t1.$eq(blending, 0))
        J.disable$1$x(this._gl, 3042);
      else if (t1.$eq(blending, 2)) {
        J.enable$1$x(this._gl, 3042);
        J.blendEquation$1$x(this._gl, 32774);
        J.blendFunc$2$x(this._gl, 770, 1);
      } else if (t1.$eq(blending, 3)) {
        J.enable$1$x(this._gl, 3042);
        J.blendEquation$1$x(this._gl, 32774);
        J.blendFunc$2$x(this._gl, 0, 769);
      } else if (t1.$eq(blending, 4)) {
        J.enable$1$x(this._gl, 3042);
        J.blendEquation$1$x(this._gl, 32774);
  ...
  buildProgram$41$alphaTest$boneTextureHeight$boneTextureWidth$bumpMap$doubleSided$envMap$flipSided$fog$fogExp$lightMap$map$maxBones$maxDirLights$maxHemiLights$maxMorphNormals$maxMorphTargets$maxPointLights$maxShadows$maxSpotLights$metal$morphNormals$morphTargets$normalMap$perPixel$shadowMapCascade$shadowMapDebug$shadowMapEnabled$shadowMapType$sizeAttenuation$skinning$specularMap$useFog$useVertexTexture$vertexColors$wrapAround: function(shaderID, fragmentShader, vertexShader, uniforms, attributes, defines, alphaTest, boneTextureHeight, boneTextureWidth, bumpMap, doubleSided, envMap, flipSided, fog, fogExp, lightMap, map, maxBones, maxDirLights, maxHemiLights, maxMorphNormals, maxMorphTargets, maxPointLights, maxShadows, maxSpotLights, metal, morphNormals, morphTargets, normalMap, perPixel, shadowMapCascade, shadowMapDebug, shadowMapEnabled, shadowMapType, sizeAttenuation, skinning, specularMap, useFog, useVertexTexture, vertexColors, wrapAround) {
    var t1, chunks, code, t2, pl, p, program, shadowMapTypeDefine, customDefines, glprogram, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19, t20, t21, t22, t23, t24, t25, t26, t27, t28, t29, t30, t31, t32, t33, t34, t35, t36, t37, t38, prefix_vertex, glFragmentShader, glVertexShader, $status, error, i;
    t1 = {};
    chunks = [];
    if (shaderID != null)
    ...
    t18 = lightMap != null;
    t19 = t18 ? "#define USE_LIGHTMAP" : "";
    t20 = bumpMap == null;
    t21 = !t20;
    t22 = t21 ? "#define USE_BUMPMAP" : "";
    t23 = specularMap != null;
    t24 = t23 ? "#define USE_SPECULARMAP" : "";
    t25 = typeof vertexColors === "boolean";
    if (!(t25 && vertexColors))
      t26 = typeof vertexColors === "number" && Math.floor(vertexColors) === vertexColors && vertexColors !== 0;
    else
      t26 = true;
    t26 = t26 ? "#define USE_COLOR" : "";
    t27 = skinning ? "#define USE_SKINNING" : "";
    t28 = useVertexTexture ? "#define BONE_TEXTURE" : "";
    t29 = morphTargets ? "#define USE_MORPHTARGETS" : "";
    t30 = morphNormals ? "#define USE_MORPHNORMALS" : "";
    t31 = perPixel ? "#define PHONG_PER_PIXEL" : "";
    t32 = wrapAround ? "#define WRAP_AROUND" : "";
    t33 = doubleSided ? "#define DOUBLE_SIDED" : "";
    t34 = flipSided ? "#define FLIP_SIDED" : "";
    t35 = shadowMapEnabled ? "#define USE_SHADOWMAP" : "";
    t36 = shadowMapEnabled ? "#define " + shadowMapTypeDefine : "";
    t37 = shadowMapDebug ? "#define SHADOWMAP_DEBUG" : "";
    t38 = shadowMapCascade ? "#define SHADOWMAP_CASCADE" : "";
    prefix_vertex = C.JSArray_methods.join$1([t2, customDefines, t3, t5, t7, t9, t10, t11, t12, t13, t14, t15, t17, "", t19, t22, "", t24, t26, t27, t28, "", "", t29, t30, t31, t32, t33, t34, t35, t36, t37, t38, sizeAttenuation ? "#define USE_SIZEATTENUATION" : "", "uniform mat4 modelMatrix;", "uniform mat4 modelViewMatrix;", "uniform mat4 projectionMatrix;", "uniform mat4 viewMatrix;", "uniform mat3 normalMatrix;", "uniform vec3 cameraPosition;", "attribute vec3 position;", "attribute vec3 normal;", "attribute vec2 uv;", "attribute vec2 uv2;", "#ifdef USE_COLOR", "attribute vec3 color;", "#endif", "#ifdef USE_MORPHTARGETS", "attribute vec3 morphTarget0;", "attribute vec3 morphTarget1;", "attribute vec3 morphTarget2;", "attribute vec3 morphTarget3;", "#ifdef USE_MORPHNORMALS", "attribute vec3 morphNormal0;", "attribute vec3 morphNormal1;", "attribute vec3 morphNormal2;", "attribute vec3 morphNormal3;", "#else", "attribute vec3 morphTarget4;", "attribute vec3 morphTarget5;", "attribute vec3 morphTarget6;", "attribute vec3 morphTarget7;", "#endif", "#endif", "#ifdef USE_SKINNING", "attribute vec4 skinIndex;", "attribute vec4 skinWeight;", "#endif", ""], "\n");
    t2 = "precision " + this.precision + " float;";
    t3 = !t20 || false ? "#extension GL_OES_standard_derivatives : enable" : "";
    t5 = t21 ? "#extension GL_OES_standard_derivatives : enable" : "";
    t7 = "#define MAX_DIR_LIGHTS " + H.S(maxDirLights);
    t9 = "#define MAX_POINT_LIGHTS " + H.S(maxPointLights);
    t10 = "#define MAX_SPOT_LIGHTS " + H.S(maxSpotLights);
    t11 = "#define MAX_HEMI_LIGHTS " + H.S(maxHemiLights);
    t12 = "#define MAX_SHADOWS " + maxShadows;
    t13 = alphaTest !== 0 ? "#define ALPHATEST " + alphaTest : "";
    t4 = t4 ? "#define GAMMA_INPUT" : "";
    t6 = t6 ? "#define GAMMA_OUTPUT" : "";
    t8 = t8 ? "#define PHYSICALLY_BASED_SHADING" : "";
    t14 = useFog && fog != null ? "#define USE_FOG" : "";
    if (useFog)
      ;
    t15 = t16 ? "#define USE_MAP" : "";
    t16 = t18 ? "#define USE_LIGHTMAP" : "";
    t17 = t21 ? "#define USE_BUMPMAP" : "";
    t18 = t23 ? "#define USE_SPECULARMAP" : "";
    if (!(t25 && vertexColors))
      t19 = typeof vertexColors === "number" && Math.floor(vertexColors) === vertexColors && vertexColors !== 0;
    else
      t19 = true;
    t19 = t19 ? "#define USE_COLOR" : "";
    t20 = metal ? "#define METAL" : "";
    t21 = perPixel ? "#define PHONG_PER_PIXEL" : "";
    t22 = wrapAround ? "#define WRAP_AROUND" : "";
    t23 = doubleSided ? "#define DOUBLE_SIDED" : "";
    t24 = flipSided ? "#define FLIP_SIDED" : "";
    t25 = shadowMapEnabled ? "#define USE_SHADOWMAP" : "";
    t26 = shadowMapEnabled ? "#define " + shadowMapTypeDefine : "";
    t27 = shadowMapDebug ? "#define SHADOWMAP_DEBUG" : "";
    glFragmentShader = this.getShader$2("fragment", C.JSArray_methods.join$1([t2, t3, customDefines, t5, t7, t9, t10, t11, t12, t13, t4, t6, t8, t14, "", t15, "", t16, t17, "", t18, t19, t20, t21, t22, t23, t24, t25, t26, t27, shadowMapCascade ? "#define SHADOWMAP_CASCADE" : "", "uniform mat4 viewMatrix;", "uniform vec3 cameraPosition;", ""], "\n") + H.S(fragmentShader));
  ...
  setTextureParameters$3: function(textureType, texture, isImagePowerOfTwo) {
    ...
    if (this._glExtensionTextureFilterAnisotropic != null && texture.type !== 15) {
      if (!J.$gt$n(texture.anisotropy, 1)) {
        t1 = texture.get$_data();
        t1 = t1.$index(t1, "__oldAnisotropy") != null;
      } else
        t1 = true;
      if (t1) {
        J.texParameterf$3$x(this._gl, textureType, 34046, P.min(texture.anisotropy, this.maxAnisotropy));
  ...
  setTexture$2: function(texture, slot) {
    var t1, image, t2, t3, isImagePowerOfTwo, glFormat, glType, mipmaps, il, i, mipmap;
    if (texture.get$needsUpdate()) {
      ...
      if (typeof texture === "object" && texture !== null && !!texture.$isCompressedTexture) {
        t1 = J.getInterceptor$asx(mipmaps);
        il = t1.get$length(mipmaps);
        if (typeof il !== "number")
          throw H.iae(il);
        i = 0;
        for (; i < il; ++i) {
          mipmap = t1.$index(mipmaps, i);
          t2 = J.getInterceptor$x(mipmap);
          J.compressedTexImage2D$7$x(this._gl, 3553, i, glFormat, t2.get$width(mipmap), t2.get$height(mipmap), 0, t2.get$data(mipmap));
        }
      } else {
        t1 = J.getInterceptor$asx(mipmaps);
        t2 = t1.get$length(mipmaps);
        if (typeof t2 !== "number")
          throw t2.$gt();
        if (t2 > 0 && isImagePowerOfTwo) {
          il = t1.get$length(mipmaps);
          if (typeof il !== "number")
            throw H.iae(il);
          i = 0;
  ...
  clampToMaxSize$2: function(image, maxSize) {
    var t1, maxDimension, newWidth, newHeight, e, t2;
    t1 = J.getInterceptor$x(image);
    if (J.$le$n(t1.get$width(image), maxSize) && J.$le$n(t1.get$height(image), maxSize))
      return image;
    maxDimension = P.max(t1.get$width(image), t1.get$height(image));
    newWidth = J.floor$0$n(J.$div$n(J.$mul$n(t1.get$width(image), maxSize), maxDimension));
    newHeight = J.floor$0$n(J.$div$n(J.$mul$n(t1.get$height(image), maxSize), maxDimension));
    e = document.createElement("canvas", null);
    t2 = J.getInterceptor$x(e);
    t2.set$width(e, newWidth);
    t2.set$height(e, newHeight);
    t2.get$context2D(e).drawImage(image, 0, 0, t1.get$width(image), t1.get$height(image), 0, 0, newWidth, newHeight);
    return e;
  },
  setCubeTexture$2: function(texture, slot) {
    var t1, isCompressed, cubeImage, t2, i, t3, t4, image, isImagePowerOfTwo, glFormat, glType, mipmaps, jl, j, mipmap;
    if (J.get$length$asx(texture.image) === 6)
      if (texture.needsUpdate) {
        t1 = texture.image;
        if (t1.get$webglTextureCube() == null) {
          t1.webglTextureCube = J.createTexture$0$x(this._gl);
          t1 = this.info.memory;
          t1.textures = t1.textures + 1;
        }
        t1 = this._gl;
        if (typeof 33984 !== "number")
          throw 33984 .$add();
        J.activeTexture$1$x(t1, 33984 + slot);
        J.bindTexture$2$x(this._gl, 34067, texture.image.get$webglTextureCube());
        t1 = this._gl;
        J.pixelStorei$2$x(t1, 37440, texture.flipY ? 1 : 0);
        isCompressed = !!texture.$isCompressedTexture;
        cubeImage = P.List_List(6, null);
        for (t1 = this.autoScaleCubemaps, t2 = !isCompressed, i = 0; i < 6; ++i) {
          t3 = t1 && t2;
          t4 = texture.image;
          if (t3)
            cubeImage[i] = this.clampToMaxSize$2(J.$index$asx(t4, i), this.maxCubemapSize);
          else
            cubeImage[i] = J.$index$asx(t4, i);
        }
        image = cubeImage[0];
        t1 = J.getInterceptor$x(image);
        t2 = t1.get$width(image);
        ...
        for (i = 0; i < 6; ++i) {
          t1 = cubeImage[i];
          if (isCompressed) {
            mipmaps = t1.get$mipmaps();
            t1 = J.getInterceptor$asx(mipmaps);
            jl = t1.get$length(mipmaps);
            if (typeof jl !== "number")
              throw H.iae(jl);
            j = 0;
            for (; j < jl; ++j) {
              mipmap = t1.$index(mipmaps, j);
              t2 = this._gl;
              if (typeof 34069 !== "number")
                throw 34069 .$add();
              t3 = J.getInterceptor$x(mipmap);
              J.compressedTexImage2D$7$x(t2, 34069 + i, j, glFormat, t3.get$width(mipmap), t3.get$height(mipmap), 0, t3.get$data(mipmap));
            }
          } else {
            t2 = this._gl;
            if (typeof 34069 !== "number")
              throw 34069 .$add();
            J.texImage2DImage$6$x(t2, 34069 + i, 0, glFormat, glFormat, glType, t1);
          }
        }
        if (texture.get$generateMipmaps() && isImagePowerOfTwo)
          J.generateMipmap$1$x(this._gl, 34067);
        texture.needsUpdate = false;
      } else {
        t1 = this._gl;
        if (typeof 33984 !== "number")
...
WebGLRenderer_addObject_closure: {"": "Closure;this_0,webglobject_1,geometry_2",
  call$2: function(k, geometryGroup) {
    ...
    if (geometryGroup.get$__webglVertexBuffer() == null) {
      ...
      t2.$indexSet(t1, "verticesNeedUpdate", true);
      t2.$indexSet(t1, "morphTargetsNeedUpdate", true);
      t2.$indexSet(t1, "elementsNeedUpdate", true);
      t2.$indexSet(t1, "uvsNeedUpdate", true);
      t2.$indexSet(t1, "normalsNeedUpdate", true);
      t2.$indexSet(t1, "tangentsNeedUpdate", true);
      t2.$indexSet(t1, "colorsNeedUpdate", true);
    }
  },
  $is_args2: true
},
...
WebGLGeometry: {"": "Object;id*,_vertices,faces3<,faces4<,materialIndex,numMorphTargets,numMorphNormals,geometryGroups<,geometryGroupsList<,__webglInit@,__inittedArrays<,__vertexArray?,__normalArray,__tangentArray,__colorArray,__lineDistanceArray,__uvArray,__uv2Array,__skinVertexAArray,__skinVertexBArray,__skinIndexArray,__skinWeightArray,__faceArray,__lineArray,__morphTargetsArrays,__morphNormalsArrays,__webglFaceCount<,__webglLineCount<,__webglParticleCount,__webglVertexCount,__sortArray,__webglCustomAttributesList<,__webglVertexBuffer<,__webglNormalBuffer,__webglTangentBuffer,__webglColorBuffer,__webglLineDistanceBuffer,__webglUVBuffer,__webglUV2Buffer,__webglSkinVertexABuffer,__webglSkinVertexBBuffer,__webglSkinIndicesBuffer,__webglSkinWeightsBuffer,__webglFaceBuffer<,__webglLineBuffer<,__webglMorphTargetsBuffers<,__webglMorphNormalsBuffers,_geometry",
  ...
  set$verticesNeedUpdate: function(flag) {
    J.$indexSet$ax(this._geometry, "verticesNeedUpdate", flag);
  },
  set$colorsNeedUpdate: function(flag) {
    J.$indexSet$ax(this._geometry, "colorsNeedUpdate", flag);
  },
  set$lineDistancesNeedUpdate: function(flag) {
    J.$indexSet$ax(this._geometry, "lineDistancesNeedUpdate", flag);
  },
...
WebGLGeometry$_internal: function(geometry) {
  return new S.WebGLGeometry(J.get$id$x(geometry), null, null, null, 0, 0, 0, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, geometry);
},

WebGLGeometry_WebGLGeometry$from: function(geometry) {
  var t1 = J.getInterceptor$asx(geometry);
  if (t1.$index(geometry, "__webglBuffer") == null)
    t1.$indexSet(geometry, "__webglBuffer", S.WebGLGeometry$_internal(geometry));
...
WebGLMaterial: {"": "Object;_material,program<,_fragmentShader,_vertexShader,_uniforms,uniformsList,numSupportedMorphTargets,numSupportedMorphNormals,shadowPass?",
  ...
  get$morphTargets: function() {
    var t1 = this._material;
    return (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || !!t1.$isShaderMaterial) && t1.get$morphTargets();
  },
  get$morphNormals: function() {
    var t1 = this._material;
    return (!!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || !!t1.$isShaderMaterial) && t1.get$morphNormals();
  },
  get$fog: function() {
  ...
  get$map: function(_) {
    var t1 = this._material;
    return !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || false ? t1.get$map(t1) : null;
  },
  map$1: function($receiver, arg0) {
  ...
  get$lightMap: function() {
    var t1 = this._material;
    return !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial ? t1.get$lightMap() : null;
  },
  get$bumpMap: function() {
    var t1 = this._material;
    return !!t1.$isMeshPhongMaterial ? H.interceptedTypeCast(t1, "$isMeshPhongMaterial").bumpMap : null;
  },
  get$normalMap: function() {
    var t1 = this._material;
    return !!t1.$isMeshPhongMaterial ? t1.get$normalMap() : null;
  },
  get$specularMap: function() {
    var t1 = this._material;
    return !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial ? t1.get$specularMap() : null;
  },
  get$wireframe: function() {
    var t1 = this._material;
    return !t1.$isLineBasicMaterial && t1.get$wireframe() === true;
  },
  ...
  get$skinning: function() {
    var t1 = this._material;
    return (!!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || !!t1.$isShaderMaterial) && t1.get$skinning();
  },
  get$size: function(_) {
    return;
  },
  ...
  get$_hasVertexColors: function() {
    var t1 = this._material;
    return !!t1.$isLineBasicMaterial || !!t1.$isMeshBasicMaterial || !!t1.$isMeshLambertMaterial || !!t1.$isMeshPhongMaterial || !!t1.$isShaderMaterial;
  },
...
CompressedTexture: {"": "Texture;mipmaps<,images,id,image,mapping,wrapS,wrapT,magFilter,minFilter,format,type,anisotropy,offset,repeat,generateMipmaps,premultiplyAlpha,needsUpdate,onUpdate,flipY,unpackAlignment,three$Texture$mipmaps,__data", $isCompressedTexture: true},
...
Texture: {"": "Object;id*,image<,mapping,wrapS,wrapT,magFilter,minFilter,format,type>,anisotropy,offset*,repeat@,generateMipmaps@,premultiplyAlpha,needsUpdate@,onUpdate,flipY>,unpackAlignment,mipmaps<,__data<",
  clone$0: function(_) {
    ...
    clonedTexture.mipmaps = P.List_List$from(this.get$mipmaps(), true, null);
...
["", "web_gl_morph_targets_horse.dart", , Y, {
main: function() {
  Y.init();
  var t1 = window;
  C.Window_methods._ensureRequestAnimationFrame$0(t1);
  C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(Y.animate$closure));
  Y.render();
},

init: function() {
  var t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, light, loader;
  ...
  t16 = new Float32Array(3);
  t16.$dartCachedLength = t16.length;
  t16 = new S.PerspectiveCamera(50, t1 / t2, null, null, null, null, null, null, t3, t4, t5, 1, 10000, t6, "", t7, null, [], t8, t9, t10, t11, "XYZ", null, null, null, true, null, t12, t13, t14, true, true, t15, false, 0, 1, true, false, false, true, new T.Vector3(t16), null, null);
  t16.updateProjectionMatrix$0();
  $.camera = t16;
  $.camera.position.storage[1] = 300;
  t16 = new Float32Array(3);
  t16.$dartCachedLength = t16.length;
  t16 = new T.Vector3(t16);
  t16.setValues$3(0, 150, 0);
  $.cameraTarget = t16;
  $.scene = S.Scene$();
  light = S.DirectionalLight$(15724543, 2, 0);
  t16 = light.position.setValues$3(1, 1, 1);
  t16.normalize$0(t16);
  t16 = $.scene;
  t16.add$1(t16, light);
  light = S.DirectionalLight$(16773103, 2, 0);
  t16 = light.position.setValues$3(-1, -1, -1);
  t16.normalize$0(t16);
  t16 = $.scene;
  t16.add$1(t16, light);
  t1 = S.Loader_addStatusElement();
  loader = new S.JSONLoader(t1, null, null, null, "anonymous");
  loader.Loader$1(true);
  loader.load$2(loader, "horse.js", new Y.init_closure());
  ...
  t1 = $.renderer;
  t1.sortObjects = false;
  J.get$nodes$x($.container)._this.appendChild(t1.canvas);
  t1 = new W._EventStream(window, C.EventStreamProvider_resize._eventType, false);
  H.setRuntimeTypeInfo(t1, [null]);
  t2 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(Y.onWindowResize$closure), t1._useCapture);
  H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
  t2._tryResume$0();
},
...
render: function() {
  var t1, t2, time, keyframe, t3, t4;
  $.theta = $.theta + 0.1;
  t1 = $.camera.position;
  t2 = $.theta;
  t1.storage[0] = 600 * Math.sin(t2 * 0.017453292519943295);
  t1 = $.camera.position;
  t2 = $.theta;
  t1.storage[2] = 600 * Math.cos(t2 * 0.017453292519943295);
  $.camera.lookAt$1($.cameraTarget);
  if ($.mesh != null) {
    time = C.JSNumber_methods.$mod(P.DateTime$_now().millisecondsSinceEpoch, 1000);
    keyframe = C.JSInt_methods.toInt$0(C.JSNumber_methods.toInt$0(Math.floor(time / 66.66666666666667)));
    t1 = $.currentKeyframe;
    if (keyframe !== t1) {
      t2 = $.mesh.morphTargetInfluences;
      t3 = $.lastKeyframe;
      t4 = t2.length;
      if (t3 < 0 || t3 >= t4)
        throw H.ioore(t2, t3);
      ...
      if (t1 < 0 || t1 >= t4)
        throw H.ioore(t2, t1);
      t2[t1] = 1;
      if (keyframe < 0 || keyframe >= t4)
        throw H.ioore(t2, keyframe);
      t2[keyframe] = 0;
      $.lastKeyframe = t1;
      $.currentKeyframe = keyframe;
    }
    t1 = $.mesh.morphTargetInfluences;
    t2 = C.JSNumber_methods.$mod(time, 66.66666666666667);
    t3 = t1.length;
    if (keyframe < 0 || keyframe >= t3)
      throw H.ioore(t1, keyframe);
    t1[keyframe] = t2 / 66.66666666666667;
    t2 = $.lastKeyframe;
    t4 = t1[keyframe];
    if (t2 < 0 || t2 >= t3)
      throw H.ioore(t1, t2);
...
init_closure: {"": "Closure;",
  call$1: function(geometry) {
    ...
    t5 = $.MaterialCount;
    $.MaterialCount = t5 + 1;
    $.mesh = S.Mesh$(geometry, new S.MeshLambertMaterial(null, t1, t2, t3, false, t4, null, null, null, null, 0, 1, 0.98, 2, false, 1, "round", "round", false, true, false, 0, true, "", t5, 0, 1, 1, 204, 205, 100, 0, false, 0, 0, false, true, true, false, true, true, null));
    $.mesh.scale.setValues$3(1.5, 1.5, 1.5);
    t5 = $.scene;
    t5.add$1(t5, $.mesh);
  },
  $is_args1: true
...
init.globalFunctions.onWindowResize$closure = Y.onWindowResize$closure = new H.Closure$1(Y.onWindowResize, "onWindowResize$closure");
init.globalFunctions.animate$closure = Y.animate$closure = new H.Closure$1(Y.animate, "animate$closure");
...
J.compressedTexImage2D$7$x = function(receiver, a0, a1, a2, a3, a4, a5, a6) {
  return J.getInterceptor$x(receiver).compressedTexImage2D$7(receiver, a0, a1, a2, a3, a4, a5, a6);
};
...
J.drawImage$5$x = function(receiver, a0, a1, a2, a3, a4) {
  return J.getInterceptor$x(receiver).drawImage$5(receiver, a0, a1, a2, a3, a4);
};
...
Isolate.$lazy($, "lib", "lib", "get$lib", function() {
  var t1, t2, t3;
  t1 = new S.Uniform("f", null, true, null);
  t1.set$value(t1, null);
  t2 = new S.Uniform("f", null, true, null);
  t2.set$value(t2, -1);
  t2 = H.fillLiteralMap(["tCube", t1, "tFlip", t2], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
  t1 = H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
  t3 = $.MaterialCount;
  $.MaterialCount = t3 + 1;
  t3 = new S.ShaderMaterial("uniform samplerCube tCube;\n            uniform float tFlip;\n            varying vec3 vViewPosition;\n            void main() {\n              vec3 wPos = cameraPosition - vViewPosition;\n              gl_FragColor = textureCube( tCube, vec3( tFlip * wPos.x, wPos.yz ) );\n            }", "varying vec3 vViewPosition;\n            void main() {\n              vec4 mPosition = modelMatrix * vec4( position, 1.0 );\n              vViewPosition = cameraPosition - mPosition.xyz;\n              gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 );\n            }", null, 2, false, 1, false, false, false, false, 0, true, null, t1, "", t3, 0, 1, 1, 204, 205, 100, 0, false, 0, 0, false, true, true, false, true, true, null);
  t3.uniforms = t2;
  return H.fillLiteralMap(["normal", "", "cube", t3], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
});
...
//# sourceMappingURL=web_gl_morph_targets_horse.dart.js.map
//@ sourceMappingURL=web_gl_morph_targets_horse.dart.js.map
