...
InputElement: {"": "HtmlElement;height%,name=,src},type=,value%,width%", $isElement: true, $isNode: true, "%": "HTMLInputElement"},
...
Color: {"": "Object;_r,_g,_b",
  ...
  setHSL$3: function(h, s, l) {
    var hue2rgb, p, q;
    if (s === 0) {
      this._b = C.JSNumber_methods.toDouble$0(l);
      this._g = C.JSNumber_methods.toDouble$0(l);
      this._r = C.JSNumber_methods.toDouble$0(l);
    } else {
  ...
  get$HSL: function() {
    var r, g, b, max, min, lightness, hue, saturation, delta, t1;
    r = this._r;
    g = this._g;
    b = this._b;
    max = P.max(P.max(r, g), b);
    min = P.min(P.min(r, g), b);
    lightness = (min + max) / 2;
    if (min === max) {
      hue = 0;
      saturation = 0;
  ...
  offsetHSL$3: function(h, s, l) {
    var hsl = this.get$HSL();
    hsl[0] = hsl[0] + h;
    hsl[1] = hsl[1] + s;
    hsl[2] = hsl[2] + l;
    this.setHSL$3(hsl[0], hsl[1], hsl[2]);
    return this;
  },
  ...
  setMeshBuffers$5: function(geometryGroup, object, hint, dispose, material) {
    ...
    if (customAttributes != null) {
      ...
      for (i = 0, offset_custom = 0, offset_customSrc = 0, value = null; i < il; ++i) {
        ...
        if (t1 === 1) {
          fl = chunk_faces3.length;
          for (f = 0, offset_custom = 0; f < fl; ++f) {
            if (f >= chunk_faces3.length)
              throw H.ioore(chunk_faces3, f);
            t1 = chunk_faces3[f];
            if (t1 < 0 || t1 >= obj_faces.length)
            ...
            face = obj_faces[t1];
            t1 = customAttribute.array;
            t2 = J.$index$asx(customAttribute.value, face.get$a(face));
            if (offset_custom >= C.JS_CONST_ZYJ(t1))
              throw H.ioore(t1, offset_custom);
            t1[offset_custom] = t2;
            t2 = customAttribute.array;
            t1 = offset_custom + 1;
            t3 = J.$index$asx(customAttribute.value, face.get$b(face));
            if (t1 >= C.JS_CONST_ZYJ(t2))
              throw H.ioore(t2, t1);
            t2[t1] = t3;
            t3 = customAttribute.array;
            t1 = offset_custom + 2;
            t2 = J.$index$asx(customAttribute.value, face.get$c(face));
            if (t1 >= C.JS_CONST_ZYJ(t3))
              throw H.ioore(t3, t1);
            t3[t1] = t2;
            offset_custom += 3;
          ...
          for (f = 0; f < fl; ++f) {
            ...
            t2 = customAttribute.array;
            t1 = offset_custom + 3;
            t3 = J.$index$asx(customAttribute.value, face.get$d(face));
            if (t1 >= C.JS_CONST_ZYJ(t2))
              throw H.ioore(t2, t1);
            t2[t1] = t3;
            offset_custom += 4;
          ...
          for (f = 0, offset_custom = 0; f < fl; ++f) {
            ...
            if (t1 < 0 || t1 >= obj_faces.length)
              throw H.ioore(obj_faces, t1);
            face = obj_faces[t1];
            v1 = J.$index$asx(customAttribute.value, face.get$a(face));
            v2 = J.$index$asx(customAttribute.value, face.get$b(face));
            v3 = J.$index$asx(customAttribute.value, face.get$c(face));
            t1 = customAttribute.array;
          ...
          for (f = 0; f < fl; ++f) {
            ...
            v2 = J.$index$asx(customAttribute.value, face.get$b(face));
            v3 = J.$index$asx(customAttribute.value, face.get$c(face));
            v4 = J.$index$asx(customAttribute.value, face.get$d(face));
            t1 = customAttribute.array;
            t2 = J.getInterceptor$x(v1);
            t3 = t2.get$x(v1);
            if (offset_custom >= C.JS_CONST_ZYJ(t1))
          ...
          pp = customAttribute.type === "c" ? ["r", "g", "b"] : ["x", "y", "z"];
          fl = chunk_faces3.length;
          for (f = 0, offset_custom = 0; f < fl; ++f) {
            if (f >= chunk_faces3.length)
              throw H.ioore(chunk_faces3, f);
...
Uniform: {"": "Object;type>,_value,_dirty,_three$_array",
  ...
  get$typedValue: function() {
    ...
    if (t1 === "fv" || t1 === "fv1") {
      ...
      t2 = false;
    if (t2) {
      t1 = J.map$1$ax(H.listTypeCast(this._value), new S.Uniform_typedValue_closure());
      ...
      if (t1 === "iv" || t1 === "iv1") {
        ...
        t2 = false;
      if (t2) {
        t1 = J.map$1$ax(H.listTypeCast(this._value), new S.Uniform_typedValue_closure0());
        ...
        this._three$_array = H.interceptedTypeCast(this._value, "$isMatrix2").get$storage();
      else if (t1 === "m3")
...
["", "web_gl_custom_attributes.dart", , N, {
main: function() {
  N.init();
  var t1 = window;
  C.Window_methods._ensureRequestAnimationFrame$0(t1);
  C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(N.animate$closure));
  N.render();
},

init: function() {
  var t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, shaderMaterial, geometry, vertices;
  t1 = $.get$WIDTH();
  t2 = $.get$HEIGHT();
  if (typeof t1 !== "number")
    throw t1.$div();
  ...
  t16 = new Float32Array(3);
  t16.$dartCachedLength = t16.length;
  t16 = new S.PerspectiveCamera(30, t1 / t2, null, null, null, null, null, null, t3, t4, t5, 1, 10000, t6, "", t7, null, [], t8, t9, t10, t11, "XYZ", null, null, null, true, null, t12, t13, t14, true, true, t15, false, 0, 1, true, false, false, true, new T.Vector3(t16), null, null);
  t16.updateProjectionMatrix$0();
  t16.position.storage[2] = 300;
  $.camera = t16;
  $.scene = S.Scene$();
  $.displacement = S.Attribute$("f", null);
  $.attributes = H.fillLiteralMap(["displacement", $.displacement], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
  t16 = new S.Uniform("f", null, true, null);
  t16.set$value(t16, 1);
  $.amplitude = t16;
  t16 = new S.Uniform("c", null, true, null);
  t16.set$value(t16, S.Color$(16720384));
  $.color = t16;
  t16 = new S.Uniform("t", null, true, null);
  t16.set$value(t16, L.loadTexture("textures/water.jpg", null, null, null));
  $.texture = t16;
  $.uniforms = H.fillLiteralMap(["amplitude", $.amplitude, "color", $.color, "texture", $.texture], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
  t16 = $.texture._value;
  t16.set$wrapT(0);
  t16.wrapS = 0;
  t16 = $.uniforms;
  t15 = $.attributes;
  t14 = document.querySelector("#vertexshader").textContent;
  t13 = document.querySelector("#fragmentshader").textContent;
  t12 = H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
  t11 = $.MaterialCount;
  $.MaterialCount = t11 + 1;
  shaderMaterial = new S.ShaderMaterial(t13, t14, null, 2, false, 1, false, false, false, false, 0, true, t15, t12, "", t11, 0, 1, 1, 204, 205, 100, 0, false, 0, 0, false, true, true, false, true, true, null);
  shaderMaterial.uniforms = t16 != null ? t16 : H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
  geometry = S.SphereGeometry$(50, 128, 64, 0, 6.283185307179586, 0, 3.141592653589793);
  geometry._dynamic = true;
  $.sphere = S.Mesh$(geometry, shaderMaterial);
  t1 = $.scene;
  t1.add$1(t1, $.sphere);
  vertices = $.sphere.geometry.vertices;
  $.noise = P.List_List$generate(J.get$length$asx(vertices), new N.init_closure(), true, null);
  J.addAll$1$ax($.displacement.value, P.List_List$filled(vertices.length, 0, null));
  t1 = S.WebGLRenderer$(true, true, null, 1, 328965, null, "highp", true, false, true);
  t1.setSize$2($.get$WIDTH(), $.get$HEIGHT());
  $.renderer = t1;
  ...
  H.setRuntimeTypeInfo(t1, [null]);
  t2 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(N.onWindowResize$closure), t1._useCapture);
  H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
  t2._tryResume$0();
},
...
render: function() {
  var time, t1, t2, i, t3, t4;
  time = P.DateTime$_now().millisecondsSinceEpoch * 0.01;
  t1 = 0.01 * time;
  t2 = $.sphere.rotation.storage;
  t2[2] = t1;
  t2[1] = t1;
  t1 = $.amplitude;
  t2 = t2[1];
  t1.set$value(t1, 2.5 * Math.sin(t2 * 0.125));
  $.color._value.offsetHSL$3(0.0005, 0, 0);
  i = 0;
  while (true) {
    t1 = J.get$length$asx($.displacement.value);
    if (typeof t1 !== "number")
      throw H.iae(t1);
    t2 = $.displacement;
    if (!(i < t1))
      break;
    t1 = t2.value;
    J.$indexSet$ax(t1, i, Math.sin(0.1 * i + time));
    t1 = $.noise;
    if (i >= t1.length)
      throw H.ioore(t1, i);
    t2 = t1[i];
    $.get$rnd().toString;
    t2 = J.$add$ns(t2, 0.5 * (0.5 - Math.random()));
    if (i >= t1.length)
      throw H.ioore(t1, i);
    t1[i] = t2;
    t2 = $.noise;
    ...
    if (i >= t2.length)
      throw H.ioore(t2, i);
    t2[i] = t1;
    t1 = $.displacement.value;
    t2 = J.getInterceptor$asx(t1);
    t3 = t2.$index(t1, i);
    t4 = $.noise;
    if (i >= t4.length)
      throw H.ioore(t4, i);
    t2.$indexSet(t1, i, J.$add$ns(t3, t4[i]));
    ++i;
  }
  t2.needsUpdate = true;
...
init.globalFunctions.onWindowResize$closure = N.onWindowResize$closure = new H.Closure$1(N.onWindowResize, "onWindowResize$closure");
init.globalFunctions.animate$closure = N.animate$closure = new H.Closure$1(N.animate, "animate$closure");
...
//# sourceMappingURL=web_gl_custom_attributes.dart.js.map
//@ sourceMappingURL=web_gl_custom_attributes.dart.js.map
